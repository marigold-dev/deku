...
| E_unit of 'a
| E_left of 'a * ('a, string) Tezos_micheline.Micheline.node
   * ('a, 'op, 'lit) expr
| E_right of 'a * ('a, string) Tezos_micheline.Micheline.node
   * ('a, 'op, 'lit) expr
...
let binds_length = function
| Binds (_, az, _) -> length az

(** val args_length : ('a1, 'a2, 'a3) args -> nat **)

let rec args_length = function
| Args_nil -> O
| Args_cons (_, _, e0) -> S (args_length e0)
