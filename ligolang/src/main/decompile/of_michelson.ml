...
let decompile_value ~raise func_or_expr program entry (ty, value) =
  let output_type =
    let entry_expression = trace_option ~raise main_entrypoint_not_found @@ Ast_typed.get_entry program entry in
    match func_or_expr with
    | Expression ->
      entry_expression.type_expression
    | Function ->
      let (_,output_type) = trace_option ~raise main_entrypoint_not_a_function @@ Ast_typed.get_t_function entry_expression.type_expression in
      output_type in
  let mini_c = trace ~raise main_decompile_michelson @@ Stacking.Decompiler.decompile_value ty value in
  let typed =  trace ~raise main_decompile_mini_c    @@ Spilling.decompile mini_c output_type in
...
let decompile_expression ~raise type_value runned_result =
  ...
  | Success (ty, value) ->
    let mini_c = trace ~raise main_decompile_michelson @@ Stacking.Decompiler.decompile_value ty value in
    let typed = trace ~raise main_decompile_mini_c @@ Spilling.decompile mini_c type_value in
