...
export class DekuToolkit {
  ...
  private pendingOperations: {
    [key: string]: {
      // TODO: replace with operationHash
  ...
  constructor(setting: Setting) {
    ...
    this.onBlockCallback = () => {
      return;
    }; // The callback is not provided by the user in the constructor
  ...
  async wait(
    ...
    const promise = new Promise<LevelType>((resolve, abort) => {
      ...
      const reject = () =>
        abort({
          type: "OPERATION_NOT_APPLIED",
          msg: "The operation has not been seen in blocks",
        });
