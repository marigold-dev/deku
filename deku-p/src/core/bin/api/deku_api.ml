...
let main params =
  ...
  let state =
    ...
    | Some state_data ->
        let Api_storage.{ protocol; current_block; receipts } =
          state_data
        in
...
let () =
  ...
  let init_term = Term.(const init_from_chain $ init_from_chain_params_cmdliner_term ()) in
  let init_cmd = Cmd.v init_info init_term in

  let cmd = Cmd.group (Cmd.info "deku-api") ~default:main_term
    [ main_cmd; init_cmd ]
  in
