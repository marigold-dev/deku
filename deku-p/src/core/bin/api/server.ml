...
let route =
  Routes.map
    (fun path ~env ~constants ~indexer ~body ->
      let body =
        try
      ...
      | Ok body -> (
          let response =
            Handler.handler ~env ~path ~body ~constants ~indexer
          in
          match response with
...
let route =
  Routes.map
    (fun path ~env ~constants ~indexer ~body:_ ->
      let response = Handler.handler ~env ~path ~constants ~indexer in
      match response with
      | Error error -> error_to_response error
      | Ok response ->
          let body = Handler.yojson_of_response response in
...
let make_handler ~env ~constants ~indexer server request =
...
| Ok body -> (
    ...
    | Ok (Routes.MatchWithTrailingSlash handler) ->
        handler ~env ~constants ~indexer ~body
    | Ok Routes.NoMatch ->
