...
test_wasm() {
  ...
  echo "param: \"$ARG\""
  sleep 5
  INVOKED=$(deku-cli create-transaction ./data/0 wallet.json $CONTRACT_ADDRESS 100 "Pair \"$DUMMY_TICKET\" 0x" --arg=$ARG --vm_flavor="Wasm" --tickets="Pair \"$DUMMY_TICKET\" 0x":100)
  sleep 5
