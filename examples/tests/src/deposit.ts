...
const run = async ({ dekuRpc, secret, tezosRpc, ticketer, data }) => {
  ...
  const operation = await contract.methods
    .mint_to_deku(consensus, address, 10, data)
    .send();
  ...
  if (previousBalance + 10 !== nextBalance)
    throw `The balance of ${address} for ticket ${ticketer} of data ${data} has not been updated`;
