...
const run = async ({dekuRpc, ligoRpc, secret}): Promise<string> => {
    ...
    const {operation, address} = await deku.originateLigo({kind: "jsligo", code, initialStorage});
    ...
    if(state !== initialStorage) throw `Invalid initial storage ${state}, expected ${initialStorage}`;
    return address;
}
