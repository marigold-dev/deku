...
const run = async ({ dekuRpc, tezosRpc }) => {
  ...
  const storage = (await contract.storage()) as {
    root_hash: { current_block_level: BigInt };
  };
  const contractLevel = Number.parseInt(
    storage.root_hash.current_block_level.toString()
  );
  ...
  if (contractLevel + 300 < dekuLevel)
    throw "The consensus is not sync, last update was 5 minutes ago.";
