...
const run = async ({tezosRpc, dekuRpc, blocks}) => {
    ...
    const lastBlocks = await Promise.all(Array(blocks).fill(0).map((elt, i) => client.getBlock({block: `head~${i}`})));
    const operations = lastBlocks
        ...
        .filter(({parameters: {entrypoint}}) => entrypoint === "update_root_hash"); // Only transaction from node are important
