...
const run = async ({
  ...
  const [sender, receiver] =
    previousBalanceA > previousBalanceB
      ? [
          { deku: dekuA, addr: aliceAddr, balance: previousBalanceA },
          { deku: dekuB, addr: bobAddr, balance: previousBalanceB },
        ]
      : [
          { deku: dekuB, addr: bobAddr, balance: previousBalanceB },
          { deku: dekuA, addr: aliceAddr, balance: previousBalanceA },
  ...
  const op = await sender.deku.transferTo(
    receiver.addr,
    1,
    ticketer,
    data.slice(2)
  ...
  const nextBalanceReceiver = await receiver.deku.getBalance(
    receiver.addr,
    ticketId
  );
  // Check that the balances have been correctly updated.
  if (sender.balance - 1 !== nextBalanceSender)
    throw "The balance of the sender has not been updated";
  if (receiver.balance + 1 !== nextBalanceReceiver)
    throw "The balance of the receiver has not been updated";
