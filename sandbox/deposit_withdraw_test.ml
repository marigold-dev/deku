...
let deposit_withdraw_test rpc_address deku_address deku_secret =
  ...
  let%ok _ = retry ~label:"deposit withdraw test: get balance" get_balance in
  ...
  let%ok _ =
    retry ~label:"depsoit withdraw test: get bigmap size" get_big_map_size in
  ...
  let%ok _ = retry ~label:"deposit withdraw test: withdraw" withdraw in
  print_endline "Deposit/Withdraw test succeeded 🎉";
  Ok ()

let deposit_withdraw_test rpc_address =
  let%ok _result = deposit_withdraw_test rpc_address deku_address deku_secret in
  Ok ()

let term =
  let open Term in
  const deposit_withdraw_test $ rpc_address
