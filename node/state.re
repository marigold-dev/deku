...
let make =
  ...
  };
  {
    ...
    next_state_root_hash: BLAKE2B.hash("Tetsutetsu Tetsutetsu"),
    // networking
...
let load_snapshot =
  ...
  let protocol =
    Protocol.{
      ...
      last_block_hash,
      state_root_hash,
      // The node uses this time to check that state root hash updates
      // are within an acceptable time frame, and thus needs to be current.
