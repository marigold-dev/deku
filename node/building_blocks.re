...
let block_has_signable_state_root_hash = (~current_time, state, block) => {
  ...
  Crypto.(
    if (BLAKE2B.equal(block.Block.state_root_hash, protocol.state_root_hash)) {
      // If the block's state root is the same as the current state root,
      // then we are in the same epoch, and we must check that the block producer
      // has not missed the deadline for the next epoch.
      time_since_last_epoch <= maximum_signable_time_between_epochs;
    } else {
      // Otherwise, the epoch has changed, and we must check two things:
      // - That the block has the expected state root hash of the next epoch
