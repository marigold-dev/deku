...
deploy_contract () {
  ...
  sleep 2
  octez_client --endpoint "$DEKU_TEZOS_RPC_NODE" originate contract "$1" \
...
export DEKU_CONSENSUS_CONTRACT="$(octez_client --endpoint $DEKU_TEZOS_RPC_NODE show known contract test_consensus | grep KT1 | tr -d '\r')"
...
deploy_contract "dummy_ticket" \
    "./dummy_ticket.mligo" \
    "()"
export DEKU_DUMMY_TICKET_CONTRACT="$(octez_client --endpoint $DEKU_TEZOS_RPC_NODE show known contract dummy_ticket | grep KT1 | tr -d '\r')"
...
ticket_wallet="bob"
octez_client --endpoint "$DEKU_TEZOS_RPC_NODE" transfer 0 from $ticket_wallet to dummy_ticket \
