...
let raise ~loc error =
  let message =
    ...
    | Extension_escape_needs_to_contain_an_expression ->
        "escapes must contain expressions"
  in
...
let parse_escape_extension ~loc (name, payload) =
  ...
  let content =
    ...
    | _ -> raise ~loc Extension_escape_needs_to_contain_an_expression
  in
  content
...
let rec expr_of_ocaml_expr expr =
  ...
  | Pexp_ident var -> (
      let name =
        match var.txt with
        | Lident name -> name
        | Ldot _ | Lapply _ -> raise ~loc Unsupported_module_idents
      in
  ...
  | Pexp_fun (label, default, param, body) ->
      ...
      let param =
        ...
        | _ ->
            ...
            raise ~loc Only_simple_var_pattern_is_supported
      in
      let param =
  ...
  | Pexp_ifthenelse (predicate, consequent, alternative) ->
      ...
      let alternative =
        ...
        | None -> raise ~loc Unsupported_if_without_else
      in
      [%expr
        If
          {
  ...
  | Pexp_tuple tuple ->
      let first, second =
        ...
        | _ -> raise ~loc Only_pair_tuple_is_supported
      in
...
let expr_rule =
  ...
  let extension =
    Extension.(declare "lambda_vm" Expression expr_pattern transform)
  in
