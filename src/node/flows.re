...
let write_state_to_file path protocol =
  ((let protocol_bin = Marshal.to_string protocol [] in
    Lwt.async
      (fun () ->
         Lwt_io.with_file ~mode:Output path
           (fun oc ->
              ((let%await () = Lwt_io.write oc protocol_bin in Lwt_io.flush oc)
              ))))
  )
type flag_node = [ `Invalid_block  | `Invalid_signature ]
type ignore =
  [ `Added_block_not_signed_enough_to_desync 
  | `Added_signature_not_signed_enough_to_request  | `Already_known_block 
  | `Already_known_signature  | `Block_not_signed_enough_to_apply 
  | `Not_current_block_producer  | `Pending_blocks 
  ...
  Lwt.async
    (fun () ->
       ((let%await block = request_block_by_hash 0 ~hash in
           let state = (!get_state) () in
           match (!received_block') state
                   (fun state -> (((!set_state) state; state)
                      )) block
           with
           | ((Ok (()))) -> await ()
           | ((Error (_err))) -> await ())
...
;;Lwt.async_exception_hook :=
    (fun exn ->
       (((Printexc.to_string exn) |>
           (Format.eprintf (("global_exception: %s\n%!")
              ));
  ...
  Lwt.async
    (fun () ->
       ((let%await snapshot = request_protocol_snapshot 0 in
           let _result = load_snapshot snapshot in await ())
       ))
let request_previous_blocks state block =
  ...
  then ((request_block ~hash:(block.Block.previous_hash))
    ...
    if not (!pending)
    then (((pending := true; request_protocol_snapshot ()))
      )
let try_to_produce_block state update_state =
  ((let%assert () =
      ...
      let state =
        append_signature state update_state ~signature ~hash:(block.hash) in
  ...
  ((let open Node in
      ...
      let signatures =
        ((prev_validators |> Validators.to_list) |>
          ...
          |>
          (List.map
             (fun address -> Address_map.find_opt address signatures_map)) in
      Lwt.async
        (fun () ->
           ((let%await () =
...
let rec try_to_apply_block state update_state block =
  ((let%assert () =
        (`Block_not_signed_enough_to_apply,
          (Block_pool.is_signed ~hash:(block.Block.hash)
      ...
      let%assert () =
        ...
        let is_new_state_root_hash =
          not
            (BLAKE2B.equal (state.protocol).state_root_hash
               block.state_root_hash) in
          ...
          (let state = clean state update_state block in
           ...
           then
             (((write_state_to_file
                  ...
                  prev_protocol;
                (match Block_pool.find_signatures ~hash:(block.hash)
                         state.block_pool
                 with
                 | ((Some (signatures))) when
                     Signatures.is_self_signed signatures ->
                     try_to_commit_state_hash
                       ~prev_validators:(prev_protocol.validators) state
                       block signatures
                 | _ -> ())))
             );
           (match Block_pool.find_next_block_to_apply
                    ~hash:(block.Block.hash) state.block_pool
            with
            | ((Some (block))) ->
...
and block_added_to_the_pool state update_state block =
  ((let state =
      match Block_pool.find_signatures ~hash:(block.Block.hash)
              state.Node.block_pool
      with
      | ((Some (signatures))) when
          Signatures.is_signed signatures ->
          let snapshots =
            Snapshots.append_block ~pool:(state.Node.block_pool)
              (block, signatures) state.snapshots in
      ...
      (([%assert
          (let () =
           ...
           let%assert () =
             ...
             match Block_pool.find_block ~hash:(block.previous_hash)
                     state.block_pool
             with
             | ((Some (block))) ->
...
let received_block state update_state block =
  ((let%ok () =
      ...
      let%assert () =
          (`Already_known_block,
            (not (is_known_block state ~hash:(block.Block.hash)))) in
...
let received_signature state update_state ~hash  ~signature  =
  ((let%assert () =
        (`Invalid_signature_for_this_hash,
          (Signature.verify ~signature hash)) in
      let%assert () =
          (`Already_known_signature,
            (not (is_known_signature state ~hash ~signature))) in
        ...
        let%assert () =
          ...
          | ((Some (block))) ->
              block_added_to_the_pool state update_state block
          | None -> (request_block ~hash; ((Ok (())))))
  )
let parse_internal_tezos_transaction transaction =
  match transaction with
  | Tezos_interop.Consensus.Update_root_hash _ ->
      ((Error (`Update_root_hash)))
  | ((Tezos_interop.Consensus.Deposit
      ({ ticket; amount; destination }))) ->
      let amount = Core.Amount.of_int (Z.to_int amount) in
      ((Ok
          (((Core.Tezos_operation.Tezos_deposit
               ({ destination; amount; ticket })))))
        )
...
let received_tezos_operation state update_state tezos_interop_operation =
  ((let open Protocol.Operation in
      let Tezos_interop.Consensus.{ hash; transactions }  =
        tezos_interop_operation in
      let tezos_operation =
        Core.Tezos_operation.make
          {
            tezos_operation_hash = hash;
            internal_operations =
              (parse_internal_tezos_transactions transactions)
          } in
...
let received_user_operation state update_state user_operation =
  ((let open Protocol.Operation in
      ...
      let operation_exists =
        List.exists (fun op -> equal op operation)
...
let received_consensus_operation state update_state consensus_operation
  ...
  ((let open Protocol.Operation in
      let%assert () =
          (`Invalid_signature,
            (Consensus.verify (state.Node.identity).key signature
  ...
  ((let%ok nonce =
      ...
      let%assert () =
          (`Invalid_nonce_signature,
            (Signature.verify ~signature (BLAKE2B.hash nonce))) in
        let _state =
          update_state
            {
              state with
              validators_uri =
                (Node.Address_map.add (Signature.address signature) uri
                   state.validators_uri)
            } in
        ((Ok (()))))
  )
let request_withdraw_proof state ~hash  =
  match state.Node.recent_operation_receipts |> (BLAKE2B.Map.find_opt hash)
  with
  | None -> Networking.Withdraw_proof.Unknown_operation
  | ((Some
      (((Receipt_tezos_withdraw (handle))))))
      ->
      let last_block_hash = (state.Node.protocol).last_block_hash in
      let handles_hash =
        match Block_pool.find_block ~hash:last_block_hash
                state.Node.block_pool
        with
        | None -> assert false
...
let trusted_validators_membership state update_state request =
  ((let open Networking.Trusted_validators_membership_change in
      ...
      let payload_hash =
        ((payload |> payload_to_yojson) |> Yojson.Safe.to_string) |>
          BLAKE2B.hash in
      let%assert () =
          (`Invalid_signature_author,
            ((Key_hash.compare (state.Node.identity).t
                (Signature.address signature))
               = 0)) in
        let%assert () =
            (`Failed_to_verify_payload,
              (payload_hash |> (Signature.verify ~signature))) in
          let new_validators =
            match action with
            | Add ->
