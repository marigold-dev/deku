...
let is_known_signature state ~hash  ~signature  =
  ((let%default () = false in
      let%some signatures =
          Block_pool.find_signatures ~hash state.Node.block_pool in
...
let block_matches_current_state_root_hash state block =
  BLAKE2B.equal block.Block.state_root_hash
...
let block_has_signable_state_root_hash ~current_time  state block =
  ((let time_since_last_epoch =
    ...
    then ((time_since_last_epoch <= maximum_signable_time_between_epochs)
      )
    else
...
let is_signable state block =
  ((let {
          ...
          protocol = { last_seen_membership_change_timestamp;_};_}
      = state in
    ...
    let is_trusted_operation operation =
      ...
      | Protocol.Operation.Core_tezos _ ->
          List.exists (fun op -> Protocol.Operation.equal op operation)
         ...
         ((is_current_producer state ~key_hash:(block.author)) &&
            ((not (has_next_block_to_apply state ~hash:(block.hash))) &&
               (all_operations_are_trusted &&
                  (block_has_signable_state_root_hash ~current_time state
                     block))))))
  )
...
let is_valid_block_height state block_height =
  (block_height >= 1L) &&
    (block_height <= (state.Node.protocol).block_height)
let signatures_required state =
  ((let number_of_validators =
      Validators.length (state.Node.protocol).validators in
    let open Float in
...
let append_signature state update_state ~hash  ~signature  =
  ((let block_pool =
      Block_pool.append_signature
        ~signatures_required:(signatures_required state) ~hash signature
...
let clean state update_state block =
  ((let operation_is_in_block operation =
      List.exists (fun op -> Operation.equal op operation)
        block.Block.operations in
    let pending_operations =
      List.find_all (fun operation -> not (operation_is_in_block operation))
        state.State.pending_operations in
    let trusted_validator_membership_change =
      List.fold_left
        (fun trusted_validator_membership_change ->
           fun operation ->
             match operation with
             | Operation.Core_tezos _ | Core_user _ ->
                 trusted_validator_membership_change
             | ((Consensus
                 (((Add_validator (validator))))))
                 ->
...
let broadcast_signature state ~hash  ~signature  =
  Lwt.async
    (fun () -> Networking.broadcast_signature state { hash; signature })
let broadcast_block_and_signature state ~block  ~signature  =
  Lwt.async
    (fun () ->
       ((let%await () = Lwt_unix.sleep 1.0 in
           Networking.broadcast_block_and_signature state
             { block; signature })
       ))
let find_random_validator_uri state =
  ((let random_int v = ((v |> Int32.of_int) |> Random.int32) |> Int32.to_int in
    ...
    let rec safe_validator_uri () =
      ((let validator =
          List.nth validators (random_int (List.length validators)) in
        if (state.Node.identity).t = validator.address
        then ((safe_validator_uri ()))
        else
          (((match Node.Address_map.find_opt validator.address
