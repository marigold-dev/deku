...
and ensure_previous_is_signed block_and_signatures t =
  ...
  | ((Some (block))) ->
      let previous_block_and_signatures =
        find_block_and_signature_or_return_empty ~hash:(block.previous_hash)
          t in
      if is_signed previous_block_and_signatures
      then ((t))
      else ((set_signed previous_block_and_signatures t)
        )
  | None -> t
let append_block block t =
  ((let block_and_signatures =
    ...
    let block_and_signatures =
      { block_and_signatures with block = ((Some (block)))
      } in
    let block_and_signatures =
      {
        ...
        signatures =
          (if block.hash = Block.genesis.hash
           then ((Signatures.set_signed block_and_signatures.signatures)
             )
           else ((block_and_signatures.signatures)))
      } in
    let t = update_block_and_signatures block_and_signatures t in
    if is_signed block_and_signatures
    then ((ensure_previous_is_signed block_and_signatures t)
      )
    else ((t)))
  )
let append_signature ~signatures_required  ~hash  signature t =
  ((let block_and_signatures =
      find_block_and_signature_or_return_empty ~hash t in
    let block_and_signatures =
      {
        block_and_signatures with
        signatures =
...
let find_block ~hash  t =
  ((let%some { block;_} = Hash_map.find_opt hash t.available in block)
  )
...
let rec find_all_signed_blocks_above blocks (block, signatures) t =
  ((match Hash_map.find_opt block.Block.hash t.signed_by_previous with
    | ((Some ((another_block, signatures)))) ->
        find_all_signed_blocks_above (block :: blocks)
          (another_block, signatures) t
