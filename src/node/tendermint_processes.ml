...
let on_timeout_propose (height : height) (round : round)
  ...
  let do_something consensus_state =
    if
      height = consensus_state.Tendermint_internals.height
      && round = consensus_state.Tendermint_internals.round
      && consensus_state.Tendermint_internals.step = Proposal
    then (
      consensus_state.Tendermint_internals.step <- Prevote;
      Broadcast
        (PrevoteOP
           ( consensus_state.Tendermint_internals.height,
             consensus_state.Tendermint_internals.round,
             nil )))
    else
      DoNothing in
...
let on_timeout_prevote (height : height) (round : round)
  ...
  let do_something consensus_state =
    if
      ...
      && consensus_state.Tendermint_internals.step = Prevote
    then (
      consensus_state.Tendermint_internals.step <- Precommit;
      Broadcast
        (PrecommitOP
           ( consensus_state.Tendermint_internals.height,
             consensus_state.Tendermint_internals.round,
...
let start_round (height : height) (round : round)
    ...
    global_state =
  consensus_state.Tendermint_internals.round <- round;
  consensus_state.Tendermint_internals.step <- Proposal;
  (* If we start a round > 0 and already have a valid/locked value, we update their hash
     with the correct round *)
  (if consensus_state.Tendermint_internals.valid_value <> nil then
     let new_value =
       update_value consensus_state.Tendermint_internals.valid_value round in
     consensus_state.Tendermint_internals.valid_value <- new_value);
  (if consensus_state.Tendermint_internals.locked_value <> nil then
     let new_value =
       update_value consensus_state.Tendermint_internals.locked_value round
     in
     consensus_state.Tendermint_internals.locked_value <- new_value);
  let return_action =
        ...
        Some
          (Broadcast
             (ProposalOP
                ( consensus_state.Tendermint_internals.height,
                  consensus_state.Tendermint_internals.round,
                  consensus_state.Tendermint_internals.valid_value,
                  consensus_state.Tendermint_internals.valid_round )))
      else
        Some
          (Broadcast
             (ProposalOP
...
let response_to_proposal (height : height) (round : round)
    ...
    =
  let _height = consensus_state.Tendermint_internals.height in
  let step = consensus_state.Tendermint_internals.step in
  let _round = consensus_state.Tendermint_internals.round in
...
let precommit_failed_previous_round (_height : height) (_round : round)
    (consensus_state : consensus_state) (msg_log : input_log) _dlog global_state
    =
  let height = consensus_state.Tendermint_internals.height in
  let step = consensus_state.Tendermint_internals.step in
...
let lock_prevote_phase (_height : height) (_round : round)
  ...
  let do_something (consensus_state : consensus_state) (valid_tuples : MySet.t)
    ...
    let round = consensus_state.Tendermint_internals.round in
    consensus_state.Tendermint_internals.valid_round <- round;
    consensus_state.Tendermint_internals.valid_value <- value;
    if consensus_state.Tendermint_internals.step = Prevote then (
...
let shortcut_prevote_fails (_height : height) (_round : round)
  ...
  let find_valid found_set : MySet.t =
    MySet.filter
      (fun (v, r) -> v = nil && r = consensus_state.Tendermint_internals.round)
      found_set in
...
let prepare_new_round_precommit_fail (height : height) (round : round)
  ...
  let found_set =
    ...
    |> MySet.filter (fun (_, r) ->
           r = consensus_state.Tendermint_internals.round) in
  let do_something () =
    Schedule
