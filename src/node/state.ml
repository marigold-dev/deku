...
let make ~identity  ~trusted_validator_membership_change 
  ...
  ((let initial_block = Block.genesis in
    ...
    let initial_snapshot = let open Snapshots in { hash; data } in
  ...
  ((let all_blocks =
    ...
    let block_pool =
      ((let block_pool =
          List.fold_left
            (fun block_pool ->
               fun block -> Block_pool.append_block block block_pool)
    ...
    let%assert () =
        (`Not_all_blocks_are_signed,
          (List.for_all
             (fun block ->
                Block_pool.is_signed ~hash:(block.Block.hash) block_pool)
             all_blocks)) in
      let%assert () =
        ...
        let%assert () =
          ...
          let of_yojson =
            [%of_yojson :
              (Core.State.t * Tezos_operation_set.t * User_operation_set.t *
                Validators.t * BLAKE2B.t * int64 * BLAKE2B.t * BLAKE2B.t)] in
          let (core_state, included_tezos_operations,
               included_user_operations, validators, validators_hash,
               block_height, last_block_hash, state_root_hash)
            =
            ((snapshot.data |> Yojson.Safe.from_string) |> of_yojson) |>
              Result.get_ok in
          let protocol =
          ...
          let%assert () =
              (`Invalid_snapshot_height,
                (protocol.block_height > (t.protocol).block_height)) in
