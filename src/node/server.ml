...
let start ~initial  =
  ...
  | Some _ ->
      failwith (("start should be called just once")
        )
  | None ->
      global_server :=
        ((Some ({ state = initial; timeout = Lwt.return_unit }))
        )
let get () =
  match !global_server with
  | ((Some (state))) -> state
  | None ->
      failwith (("get called before start")
        )
    ...
    server.timeout <-
      (([%await
          (let () = Lwt_unix.sleep 10.0 in
           (match try_to_produce_block server.state
                    (fun state -> ((server.state <- state; state)
                       ))
            with
            | ((Ok (()))) -> ()
