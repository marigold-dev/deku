...
module Wallet =
  struct
    type t = {
      ...
      priv_key: Crypto.Secret.t }[@@deriving yojson]
...
module Validators =
  struct
    ...
    let write =
      write_json
        (fun validators ->
           (validators |> (List.map (fun (address, uri) -> { address; uri })))
             |> ([%to_yojson : t list]))
  end
module Interop_context =
  struct
    module Secret =
      struct
        ...
        let of_yojson =
          ...
          | `String string ->
              (of_string string) |>
                (Option.to_result ~none:(("invalid secret")
                   ))
          | _ ->
              ((Error
                  ((("expected a string")
    ...
    type t = Tezos_interop.Context.t =
      ...
      required_confirmations: int }[@@deriving yojson]
    let read = read_json of_yojson
    let write = write_json to_yojson
  end
module State_bin =
  struct
    ...
    let write protocol ~file  =
      Lwt_io.with_file ~mode:Output file
        (fun __x -> Lwt_io.write_value __x protocol)
  end
