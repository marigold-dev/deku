...
let make ~database_uri ~consensus ~discovery =
  ...
  state := Some { indexer; discovery; consensus; websockets; random };
  Lwt.return ()
...
let register_websocket websocket t =
  let { indexer; consensus; discovery; websockets; random } = get_state () in
  let uuid = Uuidm.v4_gen t.random () in
  let websockets = Websocket.Map.add uuid websocket websockets in
  let () =
    state := Some { websockets; consensus; discovery; indexer; random }
  in
  uuid
...
let remove_websocket uuid =
  ...
  state := Some { indexer; consensus; discovery; websockets; random }
