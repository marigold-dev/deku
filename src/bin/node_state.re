  ...
  ((let%await identity =
      ...
      let%await trusted_validator_membership_change_list =
        ...
        let%await interop_context =
            Files.Interop_context.read
              ~file:(folder ^ (("/tezos.json")
                       )) in
          let%await validator_res =
              Tezos_interop.Consensus.fetch_validators
                ~context:interop_context in
            let validators =
                  ...
                  current_validators |>
                    (List.mapi
                       (fun i ->
                          fun validator ->
                            (((validator,
                                ((Printf.sprintf (("http://localhost:444%d")
                                    ) i)
                                   |> Uri.of_string)))
                            )))
            ...
            let persist_trusted_membership_change =
              Files.Trusted_validators_membership_change.write
                ~file:trusted_validator_membership_change_file in
            let node =
              State.make ~identity ~trusted_validator_membership_change
                ~interop_context ~data_folder:folder ~initial_validators_uri
            ...
            let%await state_bin_exists = Lwt_unix.file_exists state_bin in
              let%await protocol =
                  if state_bin_exists
                  then ((Files.State_bin.read ~file:state_bin)
                    )
                  else ((await node.protocol)) in
                let prev_epoch_state_bin =
                  folder ^ (("/prev_epoch_state.bin")
                    ) in
                        ...
                        ((let%await prev_protocol =
                            ...
                            let snapshots =
                              (Snapshots.add_snapshot
                                 ~new_snapshot:(let open Snapshots in
                                                  { hash; data })
