...
let contract_code_path =
  let parser file =
    ...
    non_dir_file file
    |> Result.map_error (fun _ -> `Msg "Expected path to contract JSON")
  in
...
let argument =
  ...
  let printer fmt arg =
    Format.fprintf fmt "%s" (Yojson.Safe.pretty_to_string arg)
  in
  let open Arg in
...
let address =
  let parser string =
    ...
    |> Option.to_result ~none:(`Msg "Expected a valid Deku address.")
  in
  let printer fmt wallet =
    Format.fprintf fmt "%s" (wallet |> Address.to_string)
  in
  let open Arg in
  conv (parser, printer)
...
let amount =
  let parser string =
    ...
    with _exn -> Error (`Msg "Expected an amount above zero")
  in
...
let vm_flavor =
  let parser string =
    ...
    | "Dummy" -> Some `Dummy
    | _ -> None)
    |> Option.to_result ~none:(`Msg "Expected a valid Vm_flavor")
  in
  let printer fmt flavor =
    Format.fprintf
      fmt
      "%S"
      (match flavor with `Lambda -> "Lambda" | `Dummy -> "Dummy")
  in
  let open Arg in
...
let info_create_wallet =
  let doc =
    "Creates a wallet file. The wallet file's filename is its address. The \
     wallet file contains the private uri corresponding to that address."
  in
...
let create_transaction node_folder sender_wallet_file received_address amount
  ...
  | Ok validator_uris -> (
      let validator_uris =
        List.filter_map
          (function key_hash, Some uri -> Some (key_hash, uri) | _ -> None)
          validator_uris
      in
      match validator_uris with
      | [] -> Lwt.return (`Error (false, "No validators found"))
      | (_, validator_uri) :: _ ->
          let%await block_level_response =
            request_block_level () validator_uri
          in
          ...
          let operation =
            ...
            | None, Some arg ->
                let payload =
                  ...
                  | `Lambda ->
                      Contract_vm.Invocation_payload.lambda_of_yojson ~arg
                  | `Dummy ->
                      Contract_vm.Invocation_payload.dummy_of_yojson ~arg
                in
...
let info_originate_contract =
  let doc =
    "Originates a contract. Contract origination will be communicated to all \
     known validators to be included in the next block."
  in
  Term.info
    "originate-contract"
    ~version:"%\226\128\140%VERSION%%"
...
let originate_contract =
  ...
  let vm_flavor =
    ...
    Arg.(
      ...
      & info ["vm_flavor"] ~env ~docv:"vm_flavor" ~doc)
  in
  Term.(
    lwt_ret
      (const originate_contract $ folder_node $ contract_json $ initial_storage
     $ address_from $ vm_flavor))
...
let create_transaction =
  let address_from =
    ...
    let env = Arg.env_var "SENDER" ~doc in
    let open Arg in
    required & pos 1 (some wallet) None & info [] ~env ~docv:"sender" ~doc
  in
  let address_to =
  ...
  let amount =
    ...
    let open Arg in
    required & pos 3 (some amount) None & info [] ~env ~docv:"amount" ~doc
  in
  let ticket =
  ...
  let open Term in
  lwt_ret
    (const create_transaction $ folder_node $ address_from $ address_to $ amount
   $ ticket $ argument $ vm_flavor)
...
let withdraw =
  ...
  let open Term in
  lwt_ret
    (const withdraw $ folder_node $ address_from $ tezos_address $ amount
   $ ticket)
...
let setup_tezos =
  ...
  let open Term in
  lwt_ret
    (const setup_tezos $ folder_dest $ tezos_node_uri $ tezos_secret
...
let add_trusted_validator node_folder address =
  ...
  let%await () =
    Network.request_trusted_validator_membership
      {signature; payload}
      identity.uri
  in
  await (`Ok ())
