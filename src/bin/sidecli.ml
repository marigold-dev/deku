...
let read_validators ~node_folder  =
  Files.Validators.read
    ~file:(node_folder ^ (("/validators.json")
             ))
let read_identity ~node_folder  =
  Files.Identity.read
    ~file:(node_folder ^ (("/identity.json")
             ))
let write_identity ~node_folder  =
  Files.Identity.write
    ~file:(node_folder ^ (("/identity.json")
             ))
let write_interop_context ~node_folder  =
  Files.Interop_context.write
    ~file:(node_folder ^ (("/tezos.json")))
let man =
  [`S Manpage.s_bugs;
...
let lwt_ret p = let open Term in ret ((const Lwt_main.run) $ p)
let wallet =
  ((let parser file =
      ((let non_dir_file = let open Arg in conv_parser non_dir_file in
        match ((non_dir_file file),
        ...
        | _ ->
            ((Error
                ((`Msg (("Expected path to wallet")
                    ))))
            ))
      ) in
    let printer = let open Arg in conv_printer non_dir_file in
    let open Arg in conv (parser, printer))
  )
let edsk_secret_key =
  ((let parser key =
      ((match Crypto.Secret.of_string key with
        ...
        | _ ->
            ((Error
                ((`Msg (("Expected EDSK secret key")
                    ))))
            ))
      ) in
    let printer ppf key =
      ((Format.fprintf ppf (("%s"))
          (Crypto.Secret.to_string key))
      ) in
    let open Arg in conv (parser, printer))
  )
let uri =
...
let address_implicit =
  ((let parser string =
      (Option.bind (Address.of_string string) Address.to_key_hash) |>
        (Option.to_result
           ~none:(`Msg (("Expected a wallet address.")
                    ))) in
    let printer fmt wallet =
      Format.fprintf fmt (("%s"))
        (wallet |> Key_hash.to_string) in
    let open Arg in conv (parser, printer))
  )
let address_tezos_interop =
  ((let parser string =
    ...
    let printer fmt address =
      Format.fprintf fmt (("%s"))
        (Tezos.Address.to_string address) in
    let open Arg in conv (parser, printer))
  )
let amount =
  ((let parser string =
      ((let%ok int =
            (int_of_string_opt string) |>
              (Option.to_result
                 ~none:(`Msg (("Expected an amount")
                          ))) in
          try ((Ok ((Amount.of_int int))))
          with
          | _exn ->
              ((Error
                  ((`Msg (("Expected an amount above zero")
                      ))))
              ))
      ) in
    let printer fmt amount =
      Format.fprintf fmt (("%d"))
        (Amount.to_int amount) in
    let open Arg in
      conv ~docv:(("A positive amount")
        ) (parser, printer))
  )
let tezos_required_confirmations =
  ((let msg = (("Expected an integer greater than 0")
    ...
    let parser string =
      ...
      | Some _ | None -> ((Error ((`Msg msg)))) in
    let printer fmt int =
      Format.fprintf fmt (("%d")) int in
    let open Arg in
      conv ~docv:(("An integer greater than 0")
        ) (parser, printer))
  )
let ticket =
  ((let parser string =
      (Tezos.Ticket_id.of_string string) |>
        (Option.to_result
           ~none:(`Msg (("Expected a ticket")
                    ))) in
    let printer fmt ticket =
      Format.fprintf fmt (("%S"))
        (Tezos.Ticket_id.to_string ticket) in
    let open Arg in
      conv ~docv:(("A ticket"))
        (parser, printer))
  )
let hash =
  ((let parser string =
      (BLAKE2B.of_string string) |>
        (Option.to_result
           ~none:(`Msg (("Expected a 256bits BLAKE2b hash.")
                    ))) in
    let printer fmt wallet =
      Format.fprintf fmt (("%s"))
        (BLAKE2B.to_string wallet) in
    let open Arg in conv (parser, printer))
  )
let info_create_wallet =
  ((let doc =
      (("Creates a wallet file. The wallet file's filename is its address. The wallet file contains the private uri corresponding to that address.")
      ) in
    Term.info (("create-wallet"))
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
let create_wallet () =
...
let create_wallet =
  let open Term in lwt_ret ((const create_wallet) $ (const ()))
          ...
          let%await wallet = Files.Wallet.read ~file:sender_wallet_file in
            let transaction =
              Protocol.Operation.Core_user.sign ~secret:(wallet.priv_key)
                ~nonce:0l ~block_height:block_level
                ~data:(Core.User_operation.make
            ...
            let%await identity = read_identity ~node_folder in
              let%await () =
                  Networking.request_user_operation_gossip
                    { user_operation = transaction } identity.uri in
...
let create_transaction =
  ((let address_from =
      ((let doc =
          (("The sending address, or a path to a wallet% If a bare sending address is provided, the corresponding wallet is assumed to be in the working directory.")
          ) in
        let env = Arg.env_var (("SENDER")) ~doc in
        let open Arg in
          required &
            ((pos 1 (some wallet) None) &
               (info [] ~env ~docv:(("sender"))
                  ~doc)))
      ) in
    let address_to =
      ((let doc = (("The receiving address.")
        ...
        let env =
          Arg.env_var (("RECEIVER")) ~doc in
        let open Arg in
          required &
            ((pos 2 (some address_implicit) None) &
               (info [] ~env ~docv:(("receiver")
                  ) ~doc)))
      ) in
    let amount =
      ((let doc = (("The amount to be transferred.")
        ...
        let env =
          Arg.env_var (("TRANSFER_AMOUNT")
            ) ~doc in
        let open Arg in
          required &
            ((pos 3 (some amount) None) &
               (info [] ~env ~docv:(("amount"))
                  ~doc)))
      ) in
    let ticket =
      ((let doc = (("The ticket to be transferred.")
        ...
        let open Arg in
          required &
            ((pos 4 (some ticket) None) &
               (info [] ~docv:(("MSG")) ~doc)))
      ) in
    let open Term in
      lwt_ret
        ((((((const create_transaction) $ folder_node) $ address_from) $
             address_to)
            $ amount)
           $ ticket))
  )
let info_withdraw =
  ((let doc =
      Printf.sprintf (("Submits a withdraw to the sidechain.")
        ) in
    Term.info (("withdraw"))
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
            ...
            let operation =
              Protocol.Operation.Core_user.sign ~secret:(wallet.priv_key)
                ...
                ~data:(Core.User_operation.make
                         ~sender:(Address.of_key_hash wallet.address)
                         ((Tezos_withdraw
                             ({ owner = tezos_address; amount; ticket }))
                         )) in
            let%await () =
                Networking.request_user_operation_gossip
                  { user_operation = operation } identity.uri in
              Format.printf (("operation.hash: %s\n%!")
                )
...
let withdraw =
  ((let folder_node =
    ...
    let tezos_address =
      ((let doc =
          (("The address that will be used to withdraw the ticket at Tezos **only KT1 and tz1**")
          ) in
        let open Arg in
          required &
            ((pos 2 (some address_tezos_interop) None) &
               (info [] ~docv:(("tezos_address")
                  ) ~doc)))
      ) in
    let amount =
    ...
    let open Term in
      lwt_ret
        ((((((const withdraw) $ folder_node) $ address_from) $ tezos_address)
            $ amount)
           $ ticket))
  )
  ...
  ((let open Networking in
      let%await identity = read_identity ~node_folder in
        let%await result = request_withdraw_proof { operation_hash } identity.uri in
          ...
          | Unknown_operation ->
              let message =
                (BLAKE2B.to_string operation_hash) ^ ((" is unknown")
                  ) in
              await (`Error (false, message))
          | Operation_is_not_a_withdraw ->
              let message =
                (BLAKE2B.to_string operation_hash) ^ ((" is not a withdraw")
                  ) in
              await (`Error (false, message))
      ...
      { %s })|}
                 ...
                 (Amount.to_int handle.amount) (to_hex (handle.ticket).data)
                 handle.id (Tezos.Address.to_string handle.owner)
...
let withdraw_proof =
  ((let folder_dest =
      ((let docv = (("folder_dest")) in
        let doc =
          (("The folder the files will be created in. The folder must exist and be empty.")
          ) in
        let open Arg in
          required & ((pos 0 (some string) None) & (info [] ~doc ~docv)))
      ) in
    let operation_hash =
      ((let docv = (("operation_hash")) in
        ...
        let open Arg in
          required & ((pos 1 (some hash) None) & (info [] ~doc ~docv)))
      ) in
    let contract_callback =
      ((let docv = (("contract_callback")
        ...
        let open Arg in
          required &
            ((pos 2 (some address_tezos_interop) None) & (info [] ~doc ~docv)))
      ) in
    let open Term in
      lwt_ret
        ((((const withdraw_proof) $ folder_dest) $ operation_hash) $
           contract_callback))
  )
let info_sign_block =
  ((let doc =
      (("Sign a block hash and broadcast to the network manually, useful when the chain is stale.")
      ) in
    Term.info (("sign-block"))
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
let sign_block node_folder block_hash =
  ((let%await identity = read_identity ~node_folder in
      ...
      let%await validators_uris = validators_uris node_folder in
        let%await () =
            let open Networking in
              broadcast_to_list (module Signature_spec) validators_uris
...
let info_produce_block =
  ((let doc =
      (("Produce and sign a block and broadcast to the network manually, useful when the chain is stale.")
      ) in
    Term.info (("produce-block"))
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
...
let produce_block =
  ((let folder_node =
    ...
    let open Term in lwt_ret ((const produce_block) $ folder_node))
  )
...
let setup_identity node_folder uri =
  ((let%await () = ensure_folder node_folder in
      ...
      let%await () = write_identity ~node_folder identity in await (`Ok ()))
  )
let info_setup_identity =
  ((let doc = (("Create a validator identity")
      ) in
    Term.info (("setup-identity"))
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
let setup_identity =
  ((let folder_dest =
    ...
    let self_uri =
      ((let docv = (("self_uri")) in
        let doc =
          (("The uri that other nodes should use to connect to this node.")
          ) in
        let open Arg in
          required &
            ((opt (some uri) None) &
...
let setup_tezos node_folder rpc_node secret consensus_contract
  ...
  ((let%await () = ensure_folder node_folder in
      ...
      let%await () = write_interop_context ~node_folder context in await (`Ok ()))
  )
let setup_tezos =
  ((let folder_dest =
      ((let docv = (("folder_dest")) in
    ...
    let tezos_required_confirmations =
      ((let docv = (("int")) in
        let doc =
          (("Set the required confirmations. WARNING: Setting below default of 10 can compromise security of the Deku chain.")
          ) in
        let open Arg in
          value &
    ...
    let open Term in
      lwt_ret
        ((((((const setup_tezos) $ folder_dest) $ tezos_node_uri) $
             tezos_secret)
...
let show_help =
  ((let doc = (("a tool for interacting with the WIP Tezos Sidechain")
    ...
    ((let open Term in ret (const (`Help (`Pager, None)))),
      (Term.info (("sidecli"))
         ~version:(("v0.0.1")) ~doc ~sdocs
         ~exits ~man)))
  )
let info_self =
...
let self =
  ((let folder_dest =
    ...
    let open Term in lwt_ret ((const self) $ folder_dest))
  )
let info_add_trusted_validator =
  ((let doc =
      (("Helps node operators maintain a list of trusted validators they verified off-chain which can later be used to make sure only trusted validators are added as new validators in the network.")
      ) in
    Term.info (("add-trusted-validator")
      )
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
      ...
      let%await identity = read_identity ~node_folder in
        ...
        let%await () =
            Networking.request_trusted_validator_membership
              { signature; payload } identity.uri in
          await (`Ok ()))
  )
let validator_address =
  ((let docv = (("validator_address")
    ...
    let open Arg in
      required &
        ((pos 1 (some address_implicit) None) & (info [] ~docv ~doc)))
  )
let add_trusted_validator =
  ((let open Term in
      lwt_ret
        (((const add_trusted_validator) $ folder_node) $ validator_address))
  )
let info_remove_trusted_validator =
  ((let doc =
      (("Helps node operators maintain a list of trusted validators they verified off-chain which can later be used to make sure only trusted validators are added as new validators in the network.")
      ) in
    Term.info (("remove-trusted-validator")
      )
      ~version:(("%\226\128\140%VERSION%%")
      ) ~doc ~exits ~man)
  )
