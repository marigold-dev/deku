...
module Wasm = struct
  ...
  module Interpreter = struct
    let invoke custom contract arg ~gas =
  ...
  module Interpreter = struct
    let invoke code ~ctx ~arg ~gas =
      ...
      | Lambda contract, Invocation_payload.Lambda arg ->
        ...
        let%ok contract, _ =
          Lambda.Interpreter.invoke contract ~source:M.get_source ~arg ~gas
        in
        Ok (Contract.Lambda contract, [])
      | Wasm contract, Invocation_payload.Wasm arg ->
        let module C = Wasm_vm.Ffi.Make ((val ctx)) in
        let%ok contract, operations =
          Wasm.Interpreter.invoke C.custom contract arg ~gas
          |> Result.map_error ~f:Wasm_vm.Errors.show in
        let module M = (val ctx : Contract_context.CTX) in
        let%ok ops =
          M.get_ops operations
          |> Result.map_error ~f:(fun _ -> "Invocation failure") in
        Ok (Contract.Wasm contract, ops)
