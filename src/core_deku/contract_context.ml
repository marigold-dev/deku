...
module CTX :
  Contracts.CTX
    with type Conv.Address.t = Address.t
     ...
     and type Conv.Operation.t = Contract_operation.t = struct
  module Conv = Contracts_conv
  include Contracts.State (Conv)
...
let make_state ~get_contract_opt ~source ~sender ~self ~contract_owned_tickets
  ...
  let push operation =
    ...
    idx in
  object
    inherit CTX.State.full_state

    inherit addressable ~self ~source ~sender ~get_contract_opt

    val ticket_table =
      Ticket_transition_table.init ~self ~sender ~tickets:contract_owned_tickets
