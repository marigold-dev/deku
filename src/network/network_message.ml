...
(* TODO: max size *)
let max_fragment_size = 128 * 1024
let max_fragments_size = 128 * 1024 * 1024

(* TODO: this max_size doesn't account for some stuff like separators *)
let max_fragment_size = max_fragment_size
...
module Reader : sig
  ...
  let header =
    let* size = size ~max:max_fragment_size in
    let* () = separator in
    take size
  ...
  let fragments =
    ...
    let rec loop acc remaining total buf =
      (match total > max_fragments_size with
      | true -> raise Invalid_message
      | false -> ());
      ...
      | false ->
          ...
          let fragment = fragment ~max:(max_fragments_size - total) buf in
          let total = total + String.length fragment + 1 (* separator *) in
          let acc = fragment :: acc in
          let remaining = remaining - 1 in
          loop acc remaining total buf
    in
    loop [] fragments 0
