...
let burn_gas gas vars code =
  ...
  | App _ | Const _ | Prim _ | If _ | Pair _ -> Gas.burn_constant gas
...
let compile gas script =
  try Ok (compile gas script) with Error error -> Error error
...
let compile_value gas value =
  try Ok (compile_value gas value) with Error error -> Error error
