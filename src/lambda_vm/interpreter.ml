...
let burn_gas gas env code =
  ...
  | E_lam _ | E_const _ | E_prim _ | E_if _ | E_pair _ -> Gas.burn_constant gas
...
let eval_prim context prim ~arg ~args =
  let op1_int64 f =
    let f value =
      ...
      | V_pair _ | V_closure _ | V_primitive _ -> raise Value_is_not_int64
    in
    match args with [] -> f arg | _ -> raise Over_applied_primitives
  in
  let op1_pair f =
    let f value =
      match value with
      | V_pair {first; second} -> f first second
      | V_int64 _ | V_closure _ | V_primitive _ -> raise Value_is_not_pair
    in
    match args with [] -> f arg | _ -> raise Over_applied_primitives
  in
  let op2 f =
...
let rec eval ~stack context env code =
  ...
  | E_app {funct; arg} -> (
      ...
      match funct with
      | V_pair _ | V_int64 _ -> raise Value_is_not_function
...
let execute ~context ~arg script =
  try Ok (execute ~context ~arg script) with Error error -> Result.error error
