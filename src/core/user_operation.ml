...
type t =
  ...
  hash: BLAKE2B.t ;
  sender: Address.t ;
  initial_operation: initial_operation }[@@deriving yojson]
...
let (hash, verify) =
  ((let to_yojson sender initial_operation =
      (([%to_yojson : (Address.t * initial_operation)])
         (sender, initial_operation))
...
let verify hash sender initial_operation =
  ((let%assert () =
        ((("Invalid_user_operation_hash")
          ...
          (verify hash sender initial_operation)) in
      ((Ok ({ hash; sender; initial_operation }))))
  )
let of_yojson json =
  ((let%ok t = of_yojson json in
      let%ok t = verify t.hash t.sender t.initial_operation in ((Ok (t))
          ))
  )