...
module Lambda = struct
  module Contract = struct
    type t = {code : Lambda_vm.Ir.code; storage : Lambda_vm.Ir.value}
    [@@deriving yojson, eq]
  ...
  module Origination_payload = struct
    type t = {code : Raw_repr.Script.t; storage : Raw_repr.Value.t}
    [@@deriving yojson]
  end
  ...
  module Interpreter = struct
    ...
    let invoke contract ~source ~(arg : Invocation_payload.t) ~gas =
      ...
      let%ok argument =
        ...
        |> Result.map_error compiler_error_to_string
      in
      ...
      let updated_contract =
        Contract.make ~code:contract.code ~storage:invoked.storage
      in
      Ok (updated_contract, ())
  end
end
...
module External_vm = struct
  module Contract = struct
    type t = Lambda of Lambda.Contract.t | Dummy of Dummy.Contract.t
    [@@deriving yojson, eq]
  end

  module Origination_payload = struct
    type t = Dummy of int | Lambda of Lambda.Origination_payload.t
    [@@deriving yojson]
  ...
  module Invocation_payload = struct
    type t = Lambda of Lambda.Invocation_payload.t | Dummy of (int * int)
    [@@deriving yojson]
    ...
    let invoke code ~source ~arg ~gas =
      ...
      | Contract.Dummy contract, Invocation_payload.Dummy arg ->
          let%ok contract, operations =
            Dummy.Interpreter.invoke contract ~arg ~gas
          in
          Ok (Contract.Dummy contract, operations)
      | Lambda contract, Invocation_payload.Lambda arg ->
          let%ok contract, operations =
            Lambda.Interpreter.invoke contract ~source ~arg ~gas
          in
