...
module Address_and_ticket_map =
  struct
    type key = {
      address: Key_hash.t ;
      ticket: Ticket_id.t }[@@deriving (ord, yojson)]
    module Map =
    ...
    let hash ~id  ~owner  ~amount  ~ticket  =
      ((let Ticket_id.{ ticketer; data }  = ticket in
        Tezos.Deku.Consensus.hash_withdraw_handle ~id:(Z.of_int id) ~owner
          ~amount:(Z.of_int (Amount.to_int amount)) ~ticketer ~data)
      )
  end
module Handle_tree =
  (Incremental_patricia.Make)(struct
                                type t = Handle.t[@@deriving yojson]
                                let hash t = t.Handle.hash
                              end)
type t = {
  ledger: Address_and_ticket_map.t ;
  handles: Handle_tree.t }[@@deriving yojson]
let empty =
...
let assert_available ~sender  ~amount:(amount : Amount.t)  =
  if sender >= amount
  then ((Ok (())))
      ...
      let%ok () = assert_available ~sender:sender_balance ~amount in
        ...
        ((Ok
            ({
               ledger =
                 ((t.ledger |>
                     (Address_and_ticket_map.add sender ticket
