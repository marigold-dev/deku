...
type t = {ledger : Ledger.t; contract_storage : Contract_storage.t}
[@@deriving yojson]
...
let apply_user_operation t user_operation =
  ...
  | Transaction {destination; amount; ticket} ->
      let%ok ledger =
        Ledger.transfer ~sender:source ~destination amount ticket ledger
      in
  ...
  | Contract_origination to_originate ->
      ...
      let wrap_error t =
        t |> Result.map_error (fun x -> `Origination_error x)
      in
      ...
      let contract_storage =
        Contract_storage.originate_contract
          t.contract_storage
          ~address
          ~contract
      in
