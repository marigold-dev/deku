...
open Crypto
type validator = {
  address: Key_hash.t }[@@deriving (eq, ord, yojson)]
type t =
  {
...
let update_current address t =
  ((let validator =
      t.validators |>
        (List.find_opt (fun validator -> validator.address = address)) in
    { t with current = validator })
  )
let hash_validators validators =
  (((validators |> (List.map (fun validator -> validator.address))) |>
...
let add validator t =
  ((let validators =
    ...
    {
      current = new_proposer;
      validators;
      length = (List.length validators);
      hash
    })
  )
let remove validator t =
  ((let validators = t.validators |> (List.filter ((<>) validator)) in
    ...
    let current =
      ...
      | ([], _, _) -> None
      | (_, ((Some (current))), removed) when
          current = removed -> after_current 1 t
      | _ -> t.current in
    let hash = hash_validators validators in
