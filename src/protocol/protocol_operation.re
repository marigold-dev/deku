...
module Consensus =
  struct
    type t =
      ...
      | Remove_validator of Validators.validator [@@deriving
                                                   (eq, ord, yojson)]
    let hash payload =
      ((to_yojson payload) |> Yojson.Safe.to_string) |> BLAKE2B.hash
    let sign secret t = ((let hash = hash t in Signature.sign secret hash)
      )
    let verify key signature t =
      ((let hash = hash t in Signature.verify key signature hash)
      )
  end
module Core_tezos =
  struct type t = Tezos_operation.t[@@deriving (eq, ord, yojson)] end
module Core_user =
  struct
    type t =
      {
      ...
      nonce: int32 ;
      block_height: int64 ;
      data: User_operation.t }[@@deriving (eq, yojson)]
    ...
    let (hash, verify) =
      ((let apply f ~nonce  ~block_height  ~data  =
          ((let to_yojson = [%to_yojson : (int32 * int64 * User_operation.t)] in
            ...
            let payload = Yojson.Safe.to_string json in f payload)
          ) in
        let hash = apply BLAKE2B.hash in
        let verify ~hash  = apply (BLAKE2B.verify ~hash) in (hash, verify))
      )
          ...
          let%assert () =
            ...
            let%assert () =
                ((("Invalid core_user key")
                  ...
                  (Key_hash.matches_key key sender)) in
              let%assert () =
                  ((("Invalid core_user signature")
                    ),
                    (Signature.verify key signature hash)) in
...
type t =
  ...
  | Consensus of Consensus.t [@@deriving (eq, ord, yojson)]