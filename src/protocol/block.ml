...
type t =
  ...
  author: Key_hash.t ;
  block_height: int64 ;
  operations: Protocol_operation.t list }[@@deriving yojson]
let (hash, verify) =
  ((let apply f ~state_root_hash  ~handles_hash  ~validators_hash 
      ~previous_hash  ~author  ~block_height  ~operations  =
      ((let to_yojson =
          [%to_yojson :
            (BLAKE2B.t * BLAKE2B.t * BLAKE2B.t * BLAKE2B.t * Key_hash.t *
              int64 * Protocol_operation.t list)] in
        let json =
          to_yojson
            (state_root_hash, handles_hash, validators_hash, previous_hash,
              author, block_height, operations) in
...
let make ~state_root_hash  ~handles_hash  ~validators_hash  ~previous_hash 
  ~author  ~block_height  ~operations  =
  ((let (hash, payload_hash) =
      hash ~state_root_hash ~handles_hash ~validators_hash ~previous_hash
        ~author ~block_height ~operations in
    {
      hash;
      payload_hash;
...
let genesis =
  make
    ...
    ~validators_hash:(Validators.hash Validators.empty) ~block_height:0L
    ~operations:[] ~author:(Key_hash.of_key Wallet.genesis_wallet)
let produce ~state  ~next_state_root_hash  =
  ((let next_state_root_hash =
    ...
    make ~previous_hash:(state.Protocol_state.last_block_hash)
      ...
      ~handles_hash:((Core.State.ledger state.core_state) |>
                       Ledger.handles_root_hash)
      ~validators_hash:(Validators.hash state.validators)
      ~block_height:(Int64.add state.block_height 1L))
  )
open
  (Protocol_signature.Make)(struct type nonrec t = t
                                   let hash t = t.hash end)
