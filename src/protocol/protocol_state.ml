...
let hash t =
  ((let to_yojson =
    ...
    let json =
      to_yojson
        ((t.core_state), (t.included_tezos_operations),
          (t.included_user_operations), (t.validators), (t.validators_hash),
          (t.block_height), (t.last_block_hash), (t.state_root_hash)) in
    let data = Yojson.Safe.to_string json in ((BLAKE2B.hash data), data))
  )