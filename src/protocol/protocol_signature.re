...
type t = {
  ...
  public_key: Wallet.t ;
  address: Key_hash.t }[@@deriving ord]
let public_key t = t.public_key
let address t = t.address
let signature t = t.signature
let (to_yojson, of_yojson) =
  ((let module Serialized_data =
      struct
        type t = {
          signature: Signature.t ;
          public_key: Wallet.t }[@@deriving yojson]
      end in
      let to_yojson t =
...
module Make(P:sig type t val hash : t -> BLAKE2B.t end) =
  struct
    type value = P.t
    type signature = t
    type t = {
      ...
      signature: signature }
    let sign ~key  value =
      ((let signature = (P.hash value) |> (sign ~key) in { value; signature })
      )
