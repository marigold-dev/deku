...
let apply_core_tezos_operation state tezos_operation =
  ((let%assert () =
        (`Duplicated_operation,
          (not
             (Tezos_operation_set.mem tezos_operation
                state.included_tezos_operations))) in
      let included_tezos_operations =
        Tezos_operation_set.add tezos_operation
          state.included_tezos_operations in
      let core_state =
...
let apply_core_user_operation state user_operation =
  ((let Core_user.{ hash = _; key = _; signature = _; nonce = _;
                    block_height; data } 
      = user_operation in
...
let apply_core_user_operation state tezos_operation =
  ...
  | ((Error
      ((`Block_in_the_future | `Old_operation | `Duplicated_operation)))
      ) -> (state, None)
...
let apply_operation (state, receipts) operation =
  ...
  | ((Core_user (user_operation))) ->
      ...
      let receipts =
        match receipt with
        | ((Some (receipt))) ->
            ((user_operation.hash), receipt) :: receipts
        | None -> receipts in
      (state, receipts)
  ...
  ((Printf.printf (("%Ld\n%!"))
    ...
    (let (state, receipts) =
     ...
     ({
        ...
        validators =
          (state.validators |> (Validators.update_current block.author));
