  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let size = 32
                                    let prefix =
                                      Base58.Prefix.p256_secret_key
                                    let to_raw t =
                                      Cstruct.to_string (priv_to_cstruct t)
                                    let of_raw string =
                                      (priv_of_cstruct
  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let size = 33
                                    let prefix =
                                      Base58.Prefix.p256_public_key
                                    let to_raw t =
                                      Cstruct.to_string
                                        (pub_to_cstruct ~compress:true t)
                                    let of_raw string =
                                      (pub_of_cstruct
                                         (Cstruct.of_string string))
                                        |> Result.to_option
...
module Key_hash =
  struct
    ...
    let of_key t =
      BLAKE2B_20.hash
        ((pub_to_cstruct ~compress:true t) |> Cstruct.to_string)
    include
      (Encoding_helpers.Make_b58)(struct
                                    type nonrec t = t
                                    ...
                                    let prefix =
                                      Base58.Prefix.p256_public_key_hash
  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let to_raw = Fun.id
                                    let of_raw string =
                                      match (String.length string) = size
                                      with
                                      | true -> ((Some (string))
                                          )
                                      | false -> None
...
let verify public signature hash =
  ((let (r, s) = ((String.sub signature 0 32), (String.sub signature 32 32)) in
    verify ~key:public ((Cstruct.of_string r), (Cstruct.of_string s))
