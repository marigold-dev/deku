...
module Make (P : sig
  ...
  let bin_read_t buf ~pos_ref =
    match of_raw_string_opt @@ Bin_prot.Read.bin_read_string buf ~pos_ref with
    | None -> failwith "Invalid hex"
    | Some t -> t

  let bin_size_t t = bin_size_string @@ to_raw_string t
  ...
  let bin_reader_t =
    ...
    Bin_prot.Type_class.
      { read = bin_read_t; vtag_read = variant_wrong_type "BLAKE2B" }

  let __bin_read_t__ _ =
    Log.debug "bin_read_t called";
    failwith "Not used by Pollinate"

  let bin_write_t buf ~pos t =
    Bin_prot.Write.bin_write_string buf ~pos (to_raw_string t)
