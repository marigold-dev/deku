open Helpers
module Make(P:sig val size : int end) :
  sig
  ...
  struct
    include P
    include (Digestif.Make_BLAKE2B)(struct let digest_size = P.size end)
    ...
    let of_yojson json =
      ((let%ok hex = ([%of_yojson : string]) json in
          (of_string hex) |>
            (Option.to_result ~none:(("Invalid hex")
               )))
      )
    let equal = equal
    ...
    module Map =
      (Map.Make)(struct type nonrec t = t
                        let compare = compare end)
    let encoding =
      let open Data_encoding in
        conv (fun hash -> to_raw_string hash)
          (fun string -> (string |> of_raw_string) |> Option.get)
          (Fixed.string size)
  end 
module BLAKE2B_20 = (Make)(struct let size = 20 end)
include (Make)(struct let size = 32 end)