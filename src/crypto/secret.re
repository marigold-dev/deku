...
type t =
  ...
  | P256 of P256.Secret.t [@@deriving (ord, eq)]
let to_string =
  function
  | ((Ed25519 (secret))) -> Ed25519.Secret.to_string secret
  | ((Secp256k1 (secret))) ->
      Secp256k1.Secret.to_string secret
  ...
  ((let open Data_encoding in
      ...
      let raw_encoding =
        (def (("secret_key"))
          ...
          @@
          (union
             [case ((Tag (0))) Ed25519.Secret.encoding
                ~title:(("Ed25519"))
                (function
             ...
             case ((Tag (1))) Secp256k1.Secret.encoding
               ~title:(("Secp256k1"))
               (function
                | ((Secp256k1 (secret))) ->
                    ((Some (secret)))
             ...
             case ((Tag (2))) P256.Secret.encoding
               ~title:(("P256"))
               (function
                | ((P256 (secret))) -> ((Some (secret))
                    )
                | _ -> None)
               (fun secret -> ((P256 (secret))))]) in
      Encoding_helpers.make_encoding ~name ~title ~to_string ~of_string
        ~raw_encoding)
  )
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("secret"))
    to_string of_string