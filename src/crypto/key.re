...
type t =
  ...
  | P256 of P256.Key.t [@@deriving (ord, eq)]
let of_secret =
  function
  | ((Secret.Ed25519 (secret))) ->
      ((Ed25519 ((Ed25519.Key.of_secret secret))))
  | ((Secret.Secp256k1 (secret))) ->
      ((Secp256k1 ((Secp256k1.Key.of_secret secret))))
  | ((Secret.P256 (secret))) ->
      ((P256 ((P256.Key.of_secret secret))))
let to_string =
  function
  ...
  ((let open Data_encoding in
      ...
      let raw_encoding =
        (def (("public_key"))
          ...
          @@
          (union
             [case ((Tag (0))) Ed25519.Key.encoding
                ~title:(("Ed25519"))
                (function
                 ...
                 | _ -> None)
                (fun key -> ((Ed25519 (key))));
             case ((Tag (1))) Secp256k1.Key.encoding
               ~title:(("Secp256k1"))
               (function
                | ((Secp256k1 (x))) -> ((Some (x))
                    )
                | _ -> None) (fun x -> ((Secp256k1 (x))));
             case ((Tag (2))) P256.Key.encoding
               ~title:(("P256"))
               (function
                | ((P256 (x))) -> ((Some (x))
                    )
...
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("key"))
    to_string of_string