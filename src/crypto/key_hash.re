...
type t =
  ...
  | P256 of P256.Key_hash.t [@@deriving (ord, eq)]
let of_key =
  function
  | ((Key.Ed25519 (key))) ->
      ((Ed25519 ((Ed25519.Key_hash.of_key key))))
  | ((Key.Secp256k1 (key))) ->
      ((Secp256k1 ((Secp256k1.Key_hash.of_key key))))
  | ((Key.P256 (key))) ->
      ((P256 ((P256.Key_hash.of_key key))))
...
let to_string =
  function
  | ((Ed25519 (key_hash))) ->
      Ed25519.Key_hash.to_string key_hash
  | ((Secp256k1 (key_hash))) ->
      Secp256k1.Key_hash.to_string key_hash
  ...
  ((let open Data_encoding in
      ...
      let raw_encoding =
        (def (("public_key_hash"))
          ...
          @@
          (union
             [case ((Tag (0))) Ed25519.Key_hash.encoding
                ~title:(("Ed25519"))
                (function
                 | ((Ed25519 (key_hash))) ->
                     ((Some (key_hash)))
             ...
             case ((Tag (1))) Secp256k1.Key_hash.encoding
               ~title:(("Secp256k1"))
               (function
                | ((Secp256k1 (x))) -> ((Some (x))
                    )
                | _ -> None) (fun x -> ((Secp256k1 (x))));
             case ((Tag (2))) P256.Key_hash.encoding
               ~title:(("P256"))
               (function
                | ((P256 (x))) -> ((Some (x))
                    )
                | _ -> None) (fun x -> ((P256 (x))))]) in
      Encoding_helpers.make_encoding ~name ~title ~to_string ~of_string
        ~raw_encoding)
  )
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("key_hash")
    ) to_string of_string