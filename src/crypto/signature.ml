...
let sign secret hash =
  ...
  | ((Secret.Ed25519 (secret))) ->
      ((Ed25519 ((Ed25519.sign secret hash))))
  | ((Secret.Secp256k1 (secret))) ->
      ((Secp256k1 ((Secp256k1.sign secret hash))))
  | ((Secret.P256 (secret))) ->
      ((P256 ((P256.sign secret hash))))
let verify key signature hash =
  match (key, signature) with
  | (((Key.Ed25519 (key))), ((Ed25519
     (signature)))) -> Ed25519.verify key signature hash
  | (((Key.Secp256k1 (key))), ((Secp256k1
     (signature)))) -> Secp256k1.verify key signature hash
  | (((Key.P256 (key))), ((P256
     (signature)))) -> P256.verify key signature hash
  | ((Key.Ed25519 _ | Key.Secp256k1 _ | Key.P256 _),
     (Ed25519 _ | Secp256k1 _ | P256 _)) -> false
let to_string =
  function
  | ((Ed25519 (signature))) ->
      Ed25519.Signature.to_string signature
  | ((Secp256k1 (signature))) ->
      Secp256k1.Signature.to_string signature
  | ((P256 (signature))) ->
      P256.Signature.to_string signature
let of_string =
  ((let ed25519 string =
...
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("signature")
    ) to_string of_string