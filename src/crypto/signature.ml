...
let bin_read_t buf ~pos_ref = match of_string @@ Bin_prot.Read.bin_read_string buf ~pos_ref with
  | None -> failwith "Invalid signature"
  | Some t -> t
...
let bin_reader_t =
  ...
  in
  Bin_prot.Type_class.{ read = bin_read_t; vtag_read = variant_wrong_type "Signature" }
...
let bin_shape_t = Bin_prot.Shape.(basetype (Uuid.of_string "Signature")) []
