...
module Key =
  struct
    ...
    let compare a b =
      Bigstring.compare (Libsecp256k1.Key.buffer a)
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    type nonrec t = t
                                    ...
                                    let size =
                                      Libsecp256k1.Key.compressed_pk_bytes
                                    let prefix =
                                      Base58.Prefix.secp256k1_public_key
                                    let to_raw = to_raw
                                    let of_raw string =
  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let title =
                                      (("A Secp256k1 public key hash")
                                      )
                                    let size = BLAKE2B_20.size
                                    let prefix =
                                      Base58.Prefix.secp256k1_public_key_hash
  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let prefix =
                                      Base58.Prefix.secp256k1_secret_key
                                    let to_raw t =
                                      (Libsecp256k1.Key.to_bytes context t)
                                        |> Bigstring.to_string
                                    let of_raw string =
                                      (((Libsecp256k1.Key.read_sk context
                                           (string |> Bigstring.of_string))
                                          |> Result.to_option)
                                      )
                                  end)
  ...
  struct
    ...
    include
      (Encoding_helpers.Make_b58)(struct
                                    ...
                                    let prefix =
                                      Base58.Prefix.secp256k1_signature
                                    let to_raw t =
                                      (Sign.to_bytes ~der:false context t) |>
                                        Bigstring.to_string
                                    let of_raw string =
                                      ((string |> Bigstring.of_string) |>
                                         (Sign.read context))
...
let generate () =
  ((let seed = (Random.generate 32) |> Cstruct.to_bytes in
    ...
    let pk = Libsecp256k1.Key.neuterize_exn context sk in (sk, pk))
  )
...
let verify public signature hash =
  Sign.verify_exn context ~pk:public
    ~msg:(Bigstring.of_string (BLAKE2B.to_raw_string hash)) ~signature