let unexpected_data ~name  =
  Format.kasprintf invalid_arg (("Unexpected data (%s)")
    ) name
let make_encoding ~name  ~title  ~to_string  ~of_string  ~raw_encoding  =
  ((let open Data_encoding in
      ...
      let json_encoding =
        conv to_string (Json.wrap_error of_string_exn) string in
      splitted ~binary:(obj1 (req name raw_encoding))
        ~json:(def name
  ...
  struct
    ...
    let encoding =
      make_encoding ~name ~title ~to_string ~of_string
        ~raw_encoding:(let open Data_encoding in
