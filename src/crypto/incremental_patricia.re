open Helpers
module Make(V:sig type t[@@deriving yojson] val hash : t -> BLAKE2B.t end) =
  struct
    ...
    type tree =
      ...
      left: tree ;
      hash: BLAKE2B.t ;
      right: tree } [@@deriving yojson]
    type t = {
      tree: tree ;
      top_bit: key ;
      last_key: key }[@@deriving yojson]
    ...
    let rec find bit proofs key t =
      ...
      | ((Node ({ left; right;_}))) ->
          ...
          find (bit - 1) (((hash_of_t left), (hash_of_t right)) :: proofs)
            key t
    let find key t =
      if (key >= 0) && (key <= t.last_key)
      then ((find (t.top_bit - 1) [] key t.tree))
      else ((None))
    let node left right =
    ...
    let rec empty n =
      if n = 0
      then ((Empty))
      else (((let tree = empty (n - 1) in node tree tree))
        )
    let add f t =
      ((let rec add bit key value t =
        ...
        let tree =
          ...
          then ((let right = empty (top_bit - 1) in node t.tree right)
            )
          else ((t.tree)) in
