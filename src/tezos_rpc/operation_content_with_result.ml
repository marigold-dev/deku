...
type parameters = {entrypoint : string; value : Michelson.t}
...
module Decoder = struct
  ...
  type kind = Kind_transaction | Kind_not_transaction
  ...
  let status_of_yojson json =
    ...
    match status with "applied" -> Ok Applied | _ -> Ok Other

  type parameters_base = {entrypoint : string; value : Yojson.Safe.t}
  [@@deriving of_yojson]
