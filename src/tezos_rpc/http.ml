...
type 'a method_ = GET : unit method_ | POST : string method_
...
let http_request ~node_uri ~path ~method_ response_of_yojson data =
  ...
  | Ok body -> (
      ...
      with Yojson.Json_error err -> await (Error (Json_error err)))
...
let make_lazy_lexbuf read =
  ...
  let feed s =
    read_buf := !read_buf ^ s ;
    match !waiting with Some cont -> Deep.continue cont () | None -> ()
  in
  ...
  let handler (type a) (eff : a Effect.t) :
    ...
    | Poll_data ->
        Some
          (fun (cont : (unit, unit) Deep.continuation) -> waiting := Some cont)
    | _ -> None
  in
      ...
      let body_stream =
        Lwt_stream.map
          (fun json ->
            ...
            | Ok output -> output
            | Error err ->
                raise (Request_parsing_error (Response_of_yojson err)))
          body_stream
      in
      await (Ok body_stream)
  | Error err -> await (Error err)
