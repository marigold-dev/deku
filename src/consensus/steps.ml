...
type step =
  ...
  (* verify *)
  | Check_bootstrap_signal    of Bootstrapper_signal.t
  (* transition *)
...
let check_bootstrap_signal bootstrap_signal state =
  if in_sync state then
    (state, Noop)
  else
    (state, Apply_bootstrap_signal bootstrap_signal)
