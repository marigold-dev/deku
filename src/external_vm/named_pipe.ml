  ...
  | None ->
    ...
    let vm_to_chain_permission, chain_to_vm_permission =
      if is_chain then
        ...
        ([Unix.O_WRONLY], [Unix.O_RDONLY]) in
    let vm_to_chain =
      Unix.openfile vm_to_chain_path vm_to_chain_permission 0o666 in
    let chain_to_vm =
      Unix.openfile chain_to_vm_path chain_to_vm_permission 0o666 in
    file_descriptor_map :=
