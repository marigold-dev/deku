...
module Make (CTX : Context.CTX) : S with module Context = CTX = struct
  ...
  module FFI = struct
    ...
    let load ~ctx memory writer_addr syscall =
      ...
      | READ_TICKET ->
          let _, ticket_handle =
            R.read_ticket memory writer_addr int32_offset
          in
          let store (ticket, amount, ticket_handle) =
            ...
            let content =
              Bytes.concat Bytes.empty [ ticket; buf; ticket_handle ]
            in
            Memory.store_bytes memory ~address:writer_addr ~content
          in
      ...
      | SPLIT_TICKET ->
          ...
          let store (handle1, handle2) =
            W.write_tickets ~writer_addr [ handle1; handle2 ] memory
          in
      ...
      | TRANSACTION ->
          ...
          in
          let arg, stat =
            if addr = -1L then (Bytes.empty, None)
            else
              let size =
                Bytes.get_int64_le
              ...
              in
              let bytes =
                Memory.load_bytes memory ~address:(Int64.add addr 8L) ~size
              in
              (bytes, Some Int64.(add addr 8L))
          in
          let addr, ticket_handle, address =
            R.read_ticket_with_addr memory writer_addr (int32_offset * 2)
          in
          ...
          let store int =
            ...
            Memory.store_bytes memory ~address:writer_addr ~content:buf
          in
          let called =
            Operations.transaction ctx
              (arg, (ticket_handle, amount, stat), destination)
          in
          store called
      | GET_CONTRACT_OPT ->
          let _, address = R.read_address memory writer_addr int32_offset in
          let store addr =
            W.write_some ~writer_addr addr W.write_address memory
          in
  ...
  module Wasm = struct
    ...
    module Origination_payload = struct
      type t = { code : bytes; storage : bytes } [@@deriving yojson]
    ...
    module Compiler = struct
      let compile ~gas:_ ~tickets (code : Origination_payload.t) ~sender =
        let%ok compiled =
          Wasm_vm.Module.of_string ~code:(Bytes.to_string code.code)
        in
        Ok
          Contract.
            {
  ...
  module Contract = struct
    ...
    let raw_storage t = match t with Wasm c -> c.storage
    let tickets_mapping = function Wasm c -> c.tickets |> Tickets.bindings
    ...
    let pp fmt = function Wasm t -> Wasm.Contract.pp fmt t
  end
    ...
    let compile payload ~gas ~tickets ~sender =
      ...
      | Origination_payload.Wasm contract ->
          let%ok contract =
            ...
            |> Result.map_error ~f:Wasm_vm.Errors.show
          in
                  ...
                  let () =
                    List.iter
                      ~f:(fun (t, h) ->
                        ...
                        Bytes.blit ~src:handle ~src_pos:0 ~dst:arg ~dst_pos
                          ~len:4)
                      lst
                  in
                  Ok arg
            with _ -> Error "execution_failure"
          in
