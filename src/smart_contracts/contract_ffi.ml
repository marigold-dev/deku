...
module Make (CC : Conversions.S) :
  ...
  module Addressing = struct
    ...
    let get_contract_opt t address =
      if Address.is_implicit address then Some address
      else t#get_contract_opt address |> Option.map ~f:(Fn.const address)
  end
  ...
  let make_state ~get_contract_opt ~source ~sender ~mapping ~self
    ...
    ( object
        ...
        method finalize received =
          ...
          let saved_tickets =
            finalized |> Stdlib.List.of_seq
            |> List.map ~f:(fun (key, t, a) -> ((t, a), key))
          in
            ...
            if List.equal Int.equal received deduped then
              ...
              let ops =
                List.fold_left
                  ~f:(fun acc x ->
                    let elem =
                      List.Assoc.find_exn ~equal:Int.equal operations x
                    in
                    elem :: acc)
                  ~init:[] received
              in
              Result.return ops
            else Result.fail `Execution_error
          in
