  ...
  let load ~ctx memory writer_addr syscall =
    ...
    | READ_TICKET ->
      ...
      let store (ticket, amount, ticket_handle) =
        ...
        let amount = Conv.Amount.to_int amount |> Int64.of_int in
        Bytes.set_int64_le buf 0 amount;
        let ticket = Conv.Ticket_id.to_bytes ticket in
        let ticket_handle = Conv.Ticket_handle.to_bytes ticket_handle in
