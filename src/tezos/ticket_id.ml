...
let to_string t =
  ((let loc = let open Micheline_printer in { comment = None } in
    let micheline =
      ((Micheline.Prim
          (loc, (("Pair")),
            [((String (loc, (Address.to_string t.ticketer)))
            );
            ((Bytes (loc, (t.data)))
            )], []))
      ) in
    Format.asprintf (("%a"))
      Micheline_printer.print_expr micheline)
  )
...
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("ticket_id")
    ) to_string of_string