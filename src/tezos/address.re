...
type t =
  ...
  entrypoint: string option } [@@deriving (eq, ord)]
let to_string =
  function
  ...
  | ((Originated
      ({ contract; entrypoint = ((Some (entrypoint))) }))
      ) ->
      (Contract_hash.to_string contract) ^
        ((("%")) ^ entrypoint)
let of_string =
  ((let implicit string =
    ...
    let originated string =
      ((let%some (contract, entrypoint) =
            ...
            | contract::entrypoint::[] when
                ((String.length entrypoint) < 32) &&
                  (entrypoint <> (("default")))
                ->
          ...
          let%some contract = Contract_hash.of_string contract in
            ((Some
                (((Originated ({ contract; entrypoint })))))
              ))
      ) in
...
let encoding =
  ((let open Data_encoding in
      let contract_encoding =
        (((def (("contract_id"))
            ...
            (union ~tag_size:`Uint8
               [case ((Tag (0))) ~title:(("Implicit")
                  ) Key_hash.encoding
                  (function
                   | ((Implicit (k))) -> ((Some (k))
                       )
                   | _ -> None) (fun k -> ((Implicit (k))));
               case ((Tag (1)))
                 (Fixed.add_padding Contract_hash.encoding 1)
                 ~title:(("Originated"))
                 (function
                  | ((Originated ({ contract;_}))) ->
                      ((Some (contract)))
                  | _ -> None)
                 (fun contract ->
                    ((Originated ({ contract; entrypoint = None }))
      ...
      let name = (("address")) in
      let title =
        (("An contract address optionally followed by an entrypoint.")
        ) in
      let raw_encoding =
        conv
          (fun t ->
...
let (to_yojson, of_yojson) =
  Yojson_ext.with_yojson_string (("address"))
    to_string of_string