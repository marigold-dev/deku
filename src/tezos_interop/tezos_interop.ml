...
module Run_contract =
  struct
    type input =
      ...
      destination: string ;
      entrypoint: string ;
      payload: Yojson.Safe.t }[@@deriving to_yojson]
    type output =
      | Applied of {
      hash: string } 
      | Failed of {
    ...
    let output_of_yojson json =
      ((let module T =
          struct
            type t = {
              status: string }[@@deriving of_yojson { strict = false }]
            and finished = {
              hash: string }
            and error = {
              error: string }
    ...
    let file =
      ((let%await (file, oc) =
            Lwt_io.open_temp_file ~suffix:((".js")
              ) () in
          let%await () =
              Lwt_io.write oc
                ([%blob
    ...
    let run ~context  ~destination  ~entrypoint  ~payload  =
      ((let input =
        ...
        let command = (("node")) in
        let%await output =
            Lwt_process.pmap (command, [|command;file|])
...
module Fetch_storage :
  sig
    val run :
      rpc_node:Uri.t ->
        confirmation:int ->
          contract_address:Address.t -> (michelson, string) result Lwt.t
  end =
  struct
    type input =
      {
      rpc_node: string ;
      confirmation: int ;
      contract_address: string }[@@deriving to_yojson]
    let output_of_yojson json =
      ((let module T =
          struct
            type t = {
...
module Listen_transactions =
  struct
    type transaction = {
      entrypoint: string ;
      value: michelson }[@@deriving of_yojson]
    type output = {
      hash: string ;
      transactions: transaction list }[@@deriving of_yojson]
    module CLI =
      struct
        type input =
          {
          rpc_node: string ;
          confirmation: int ;
          destination: string }[@@deriving to_yojson]
        let file =
          ((let%await (file, oc) =
                Lwt_io.open_temp_file ~suffix:((".js")
                  ) () in
          ...
          ((let send f pr data =
            ...
            let input =
              ({
                 ...
                 confirmation = (context.required_confirmations);
                 destination = (Address.to_string destination)
               } |> input_to_yojson) |> Yojson.Safe.to_string in
            let on_fail _exn =
              ((let%await _status = process#close in on_fail ())
              ) in
                ...
                Lwt.catch
                  (fun () ->
                     ((let%await line = Lwt_io.read_line process#stdout in
                         print_endline line;
                         (((Yojson.Safe.from_string line) |> output_of_yojson)
                            |> Result.get_ok)
                           |> on_message;
    ...
    let listen ~context  ~destination  ~on_message  =
      ((let rec start () =
          Lwt.catch
            ...
            (fun _exn -> on_fail ())
        and on_fail () = start () in Lwt.async start)
      )
  end
module Consensus =
  struct
    ...
    let commit_state_hash ~context  ~block_height  ~block_payload_hash 
      ...
      ((let module Payload =
          struct
            type t =
              ...
              current_validator_keys: string option list }[@@deriving
                                                            to_yojson]
          end in
          let open Payload in
    ...
    let parse_transaction transaction =
      match ((transaction.Listen_transactions.entrypoint),
              (transaction.value))
      with
      | ((("update_root_hash")),
         ((Tezos_micheline.Micheline.Prim
         (_, Michelson_v1_primitives.D_Pair, ((Prim
          (_, D_Pair, ((Prim
           (_, D_Pair, ((Bytes (_, _block_hash)))::((Int
            (_, _block_height)))::[], _)))::((Prim
           (_, D_Pair, ((Bytes
            (_, _block_payload_hash)))::((Int
            (_, _handles_hash)))::[], _)))::[],
           _)))::((Prim
          (_, D_Pair, ((Prim
           (_, D_Pair, _signatures::((Bytes
            (_, state_root_hash)))::[], _)))::_::[],
           _)))::[],
          _)))) ->
      ...
      | ((("deposit")), ((Micheline.Prim
         (_, Michelson_v1_primitives.D_Pair, ((Bytes
          (_, destination)))::((Prim
          (_, D_Pair, ((Bytes (_, ticketer)))::((Prim
           (_, D_Pair, ((Bytes (_, data)))::((Int
            (_, amount)))::[], _)))::[],
           _)))::[],
          _)))) ->
          let%some destination =
            ...
            let%some ticketer =
              ...
              let ticket = let open Ticket_id in { ticketer; data } in
              ((Some
                  (((Deposit ({ ticket; destination; amount }))
                    )))
                )
      | _ -> None
    let parse_operation output =
      ((let%some hash = Operation_hash.of_string output.Listen_transactions.hash in
          let transactions =
            List.filter_map parse_transaction output.transactions in
    ...
    let listen_operations ~context  ~on_operation  =
      ((let on_message output =
          ...
          | None -> () in
        Listen_transactions.listen ~context
          ~destination:(context.consensus_contract) ~on_message)
      )
              ...
              ((List.fold_left_ok
                  (fun acc ->
                     fun k ->
                       ...
                       | _ ->
                           ((Error
                               ((("Some key_hash wasn't of type string")
                                 )))
                           )) [] (List.rev key_hashes))
              )
          | Ok _ ->
              ((Error
                  ((("Failed to parse storage micheline expression")
                    )))
              )
          | ((Error (msg))) -> ((Error (msg))
              ) in
