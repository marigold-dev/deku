...
module Inject_transaction = struct
  ...
  let of_yojson json =
    let module T = struct
      ...
      type with_hash = {hash : string} [@@deriving of_yojson {strict = false}]
...
let listen_transaction t ~rpc_node ~required_confirmations ~destination =
  ...
  in
  Long_lived_js_process.listen
    t
    ~to_yojson:Listen_transaction.request_to_yojson
    ~of_yojson:Listen_transaction.of_yojson
    request
...
let inject_transaction t ~rpc_node ~secret ~required_confirmations ~destination
  ...
  in
  Long_lived_js_process.request
    t
    ~to_yojson:Inject_transaction.request_to_yojson
    ~of_yojson:Inject_transaction.of_yojson
    request
...
let storage t ~rpc_node ~required_confirmations ~destination =
  ...
  Long_lived_js_process.request
    t
    ~to_yojson:Storage.request_to_yojson
    ~of_yojson:Storage.of_yojson
    request
...
let big_map_keys t ~rpc_node ~required_confirmations ~destination ~keys =
  ...
  Long_lived_js_process.request
    t
    ~to_yojson:Big_map_keys.request_to_yojson
    ~of_yojson:Big_map_keys.of_yojson
    request
