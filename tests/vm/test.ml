...
let test_increment () =
  let script =
    Ast.{ param = "x"
        ; code = Pair { first = App { funct = App { funct = Prim Add
                                                  ; arg = Var "x" }
                                      ; arg = Const 1L }
...
let test_decrement () =
  let script =
    Ast.{ param = "x"
        ; code = Pair { first = App { funct = App { funct = Prim Sub
                                                  ; arg = Var "x" }
                                    ; arg = Const 1L  }
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
...
let test_add_pair () =
  ...
  let result =
    Vm_test.execute_ast_exn 2501 (Pair (Int64 23L, Int64 28L)) script
  in
  let expected_value =
...
let test_if_expr () =
  let script =
    Ast.{ param = "param"
        ; code =
            Pair { first =
                     If { predicate = Fst (Var "param")
                        ; consequent = App { funct =
                                               App { funct = Prim Add
                                                   ; arg = Snd (Var "param") }
                                           ; arg = Const 1L }
                        ; alternative = App { funct =
                                                App { funct = Prim Sub
                                                    ; arg = Snd (Var "param") }
                                            ; arg = Const 1L } }
                 ; second = Pair { first = Const 0L
  ...
  (* Check increment *)
  let result =
    Vm_test.execute_ast_exn 3501 (Pair (Int64 1L, Int64 51L)) script
  in
  let expected_value =
  ...
  (* Check decrement *)
  let result =
    Vm_test.execute_ast_exn 3501 (Pair (Int64 0L, Int64 33L)) script
  in
  let expected_value =
...
let test_lambda () =
  let script =
    let increment_lambda =
      Ast.(Lam ("x", App { funct = App { funct = Prim Add
                                       ; arg = Var "x" }
                         ; arg = Const 1L }))
    in
    let decrement_lambda =
      Ast.(Lam ("x", App { funct = App { funct = Prim Sub
                                       ; arg = Var "x" }
                         ; arg = Const 1L }))
    in
    Ast.{ param = "param"
  ...
  (* Check increment *)
  let result = Vm_test.execute_ast_exn 6101 (Pair (Int64 1L, Int64 99L)) script in
  let expected_value = Vm_test.compile_value_exn (Gas.make ~initial_gas:101) (Int64 100L) in

  Alcotest.(check Testable.value) "Same value" expected_value result.storage;

  (* Check decrement *)
  let result = Vm_test.execute_ast_exn 6101 (Pair (Int64 0L, Int64 33L)) script in
  let expected_value = Vm_test.compile_value_exn (Gas.make ~initial_gas:101) (Int64 32L) in
...
let check_execution_error result expected_error =
  let open Vm_test in
  match result with
  | Error (Execution_error error) -> Alcotest.(check Testable.execution_error) "Same error" expected_error error
  | Ok _ -> Alcotest.fail "Ast shouldn't execute"
  | Error (Compilation_error error) -> Alcotest.fail (Vm_test.compilation_error_to_string error)

let check_compilation_error result expected_error =
  let open Vm_test in
  match result with
  | Error (Compilation_error error) -> Alcotest.(check Testable.compilation_error) "Same error" expected_error error
  | Ok _ -> Alcotest.fail "Ast shouldn't execute"
  | Error (Execution_error error) -> Alcotest.fail (Vm_test.execution_error_to_string error)
...
let test_compilation_undefined_variable () =
  let script =
    Ast.{ param = "_"
        ; code = Var "x" }
  in
  check_compilation_error (Vm_test.execute_ast 2000 (Int64 0L) script) Undefined_variable

let test_fst_value_is_not_pair () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = Fst (Const 1L)
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 2000 (Int64 0L) script) Value_is_not_pair

let test_snd_value_is_not_pair () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = Snd (Const 1L)
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 2000 (Int64 0L) script) Value_is_not_pair
...
let test_op1_value_is_not_int64 () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = App { funct = Prim Neg
                                    ; arg = (Pair { first = Const 0L ; second = Const 0L }) }
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 2000 (Int64 0L) script) Value_is_not_int64
    ...
    Ast.{ param = "_"
        ; code = Pair { first = App { funct = App { funct = Prim Add
                      ...
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 3000 (Int64 0L) script) Value_is_not_int64

let test_if_value_is_not_int64 () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = If { predicate = Pair { first = Const 0L ; second = Const 0L }
                                   ; consequent = Const 1L
...
let test_value_is_not_function () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = App { funct = Const 0L
                                    ; arg = Const 0L }
                      ; second = Pair { first = Const 0L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 3000 (Int64 0L) script) Value_is_not_function

let test_pattern1_value_is_not_pair () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = Const 0L
                      ; second = Const 0L } }
  in
  check_execution_error (Vm_test.execute_ast 3000 (Int64 0L) script) Value_is_not_pair
...
let test_pattern3_value_is_not_zero () =
  let script =
    Ast.{ param = "_"
        ; code = Pair { first = Const 0L
                      ; second = Pair { first = Const 1L
                                      ; second = Const 0L } } }
  in
  check_execution_error (Vm_test.execute_ast 3000 (Int64 0L) script) Value_is_not_zero
...
let () =
  ...
  run "Lambda VM" [
    ...
    ; "Compilation and execution errors",
      [ test_case "Compilation - Undefined variable" `Quick test_compilation_undefined_variable
      ...
      ; test_case "Op1 - Value should be int64" `Quick test_op1_value_is_not_int64
    ...
    ; "Execution pattern errors",
      [ test_case "Pattern 1 - Value should be pair" `Quick test_pattern1_value_is_not_pair
      ; test_case "Pattern 2 - Value should be pair" `Quick test_pattern2_value_is_not_pair
      ; test_case "Pattern 3 - Value should be zero" `Quick test_pattern3_value_is_not_zero
      ; test_case "Pattern 4 - Value should be zero" `Quick test_pattern4_value_is_not_zero ] ]
