         ...
         let user_operations =
           List.init (batch_size |> Float.to_int) (fun _ ->
               make_transaction ~block_level:0L ~ticket:(make_ticket ticketer)
                 ~sender:alice_wallet ~recipient:bob_wallet ~amount:0 )
         in
         let batch =
           {user_operations} |> Network.User_operations_noop.request_to_yojson
