...
let make_ticket ticketer =
  ...
  let ticketer =
    Ticket_id.Tezos (Tezos.Address.Originated { contract; entrypoint = None })
  in
  Ticket_id.{ ticketer; data = Bytes.empty }
...
let load_test_transactions _test_kind ticketer =
  ...
  (* Without proper gossip, user operation broadcast ends up DOSing
     the nodes. Until we have proper gossip, we just disable this broadcast.
     Consensus is theoretically sound without, but user operations could get dropped.
     TODO: proper gossip. *)
  Format.printf
    "========= Did you remember to apply the patch file??? =========\n%!";
