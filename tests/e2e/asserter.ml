...
let assert_state_correct data_folder expected_state_root_hash
  ...
  Format.printf
    "Minimum expected block height: %Ld. Actual height: %Ld\n%!"
    expected_block_height
    protocol_state.block_height ;
  assert (protocol_state.block_height >= expected_block_height) ;
  let actual_state_root_hash =
    protocol_state.state_root_hash |> BLAKE2B.to_string
  in
  Format.printf
    "Expected state root hash: %s. Actual state root hash: %s\n%!"
...
let args =
  ...
  let open Term in
  const assert_state_correct $ folder_node $ expected_state_root_hash
  $ expected_block_height
