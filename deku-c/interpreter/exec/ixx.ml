...
struct
   ...
   *)
  let cmp_u x op y = op (Rep.add x Rep.min_int) (Rep.add y Rep.min_int)
  ...
  let shift f x y = f x Rep.(to_int (logand y (of_int (bitwidth - 1))))
  let shl x y = sx (shift Rep.shift_left x y)
  ...
  let shr_u x y = sx (shift Rep.shift_right_logical (as_unsigned x) y)
  ...
  let ctz x =
    let rec loop acc n =
      ...
      else loop (1 + acc) (Rep.shift_right_logical n 1)
    in
    Rep.of_int (loop 0 x)
