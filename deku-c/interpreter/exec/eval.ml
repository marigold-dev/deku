...
let rec step (c : config) : config =
  ...
  let vs', es' =
    ...
    | Plain e', vs -> (
        match (e', vs) with
        | Unreachable, vs -> (vs, [ Trapping "unreachable executed" @@ e.at ])
        | Nop, vs -> (vs, [])
        | Block (bt, es'), vs ->
        ...
        | Return, vs -> ([], [ Returning vs @@ e.at ])
        | Call x, vs -> (vs, [ Invoke (func frame.inst x) @@ e.at ])
        ...
        | Select _, Num (I32 i) :: v2 :: v1 :: vs' ->
            if i = 0l then (v2 :: vs', []) else (v1 :: vs', [])
        | LocalGet x, vs -> (!(local frame x) :: vs, [])
        | LocalSet x, v :: vs' ->
            local frame x := v;
            (vs', [])
        | LocalTee x, v :: vs' ->
            local frame x := v;
            (v :: vs', [])
        | GlobalGet x, vs -> (Global.load (global frame.inst x) :: vs, [])
        | GlobalSet x, v :: vs' -> (
            try
              Global.store (global frame.inst x) v;
              (vs', [])
            with
              ...
              let v =
                ...
                | Pack8 ->
                    V128.I8x16.replace_lane j v
                      (I32Num.of_num 0
                         (Memory.load_num_packed Pack8 SX mem addr offset
                            I32Type))
                | Pack16 ->
                    V128.I16x8.replace_lane j v
                      (I32Num.of_num 0
                         (Memory.load_num_packed Pack16 SX mem addr offset
                            I32Type))
                | Pack32 ->
                    V128.I32x4.replace_lane j v
                      (I32Num.of_num 0
                         (Memory.load_num mem addr offset I32Type))
                | Pack64 ->
                    V128.I64x2.replace_lane j v
                      (I64Num.of_num 0
                         (Memory.load_num mem addr offset I64Type))
              in
              (Vec (V128 v) :: vs', [])
            with exn -> (vs', [ Trapping (memory_error e.at exn) @@ e.at ]))
        | ( VecStoreLane ({ offset; ty; pack; _ }, j),
            ...
            try
              (match pack with
              | Pack8 ->
                  Memory.store_num_packed Pack8 mem addr offset
                    (I32 (V128.I8x16.extract_lane_s j v))
              | Pack16 ->
                  Memory.store_num_packed Pack16 mem addr offset
                    (I32 (V128.I16x8.extract_lane_s j v))
              | Pack32 ->
                  Memory.store_num mem addr offset
        ...
        | MemoryCopy, Num (I32 n) :: Num (I32 s) :: Num (I32 d) :: vs' ->
            if mem_oob frame (0l @@ e.at) s n || mem_oob frame (0l @@ e.at) d n
            then (vs', [ Trapping (memory_error e.at Memory.Bounds) @@ e.at ])
            else if n = 0l then (vs', [])
            else if I32.le_u d s then
              ( vs',
                List.map (at e.at)
