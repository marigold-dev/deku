...
module Make (Rep : RepType) : S with type bits = Rep.t = struct
  ...
  let determine_binary_nan x y =
     ...
     * pos_nan or neg_nan.
     *)
    let nan = if is_nan x then x else if is_nan y then y else Rep.pos_nan in
  ...
  let of_signless_string s =
    if s = "inf" then pos_inf
      ...
      if x = Rep.zero then raise (Failure "nan payload must not be zero")
  ...
  let group_digits is_digit n s =
    ...
    let point =
      Lib.Option.get (Lib.String.find_from_opt isnt_digit s mant) len
    in
