...
module V128Op = struct
  ...
  let vunop (op : vunop) = match op with Not -> "not"
...
let vec_oper vop op = match op with V128 o -> "v128." ^ vop o
...
let vec_shiftop =
  vec_shape_oper (V128Op.ishiftop, V128Op.ishiftop, V128Op.voidop)

let vec_bitmaskop =
  vec_shape_oper (V128Op.ibitmaskop, V128Op.ibitmaskop, V128Op.voidop)
...
let vec_extractop =
  vec_shape_oper (V128Op.pextractop, V128Op.extractop, V128Op.extractop)

let vec_replaceop =
  vec_shape_oper (V128Op.replaceop, V128Op.replaceop, V128Op.replaceop)
...
let func_with_name name f =
  ...
  Node
    ( "func" ^ name,
      [ Node ("type " ^ var ftype, []) ]
      @ decls "local" locals @ list instr body )
...
let table off i tab =
  ...
  Node ("table $" ^ nat (off + i) ^ " " ^ limits nat32 lim, [ atom ref_type t ])
...
let typedef i ty = Node ("type $" ^ nat i, [ struct_type ty.it ])
...
let import_desc fx tx mx gx d =
  ...
  | TableImport t ->
      incr tx;
      table 0 (!tx - 1) ({ ttype = t } @@ d.at)
  | MemoryImport t ->
      incr mx;
      memory 0 (!mx - 1) ({ mtype = t } @@ d.at)
  | GlobalImport t ->
      incr gx;
...
let action mode act =
  ...
  | Get (x_opt, name) -> Node ("get" ^ access x_opt name, [])

let nan = function
  | CanonicalNan -> "nan:canonical"
  | ArithmeticNan -> "nan:arithmetic"

let nanop (n : nanop) = match n.it with F32 n' | F64 n' -> nan n' | _ -> .
...
let assertion mode ass =
  ...
  | AssertUnlinkable (def, re) ->
      [
        Node
          ("assert_unlinkable", [ definition mode None def; Atom (string re) ]);
      ]
