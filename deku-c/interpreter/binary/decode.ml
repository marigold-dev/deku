...
let name s =
  let pos = pos s in
  try Utf8.decode (string s)
  with Utf8.Utf8 -> error s pos "malformed UTF-8 encoding"
...
let import_section s = section `ImportSection (vec (at import)) [] s

(* Function section *)

let func_section s = section `FuncSection (vec (at var)) [] s
...
let table_section s = section `TableSection (vec (at table)) [] s
...
let memory_section s = section `MemorySection (vec (at memory)) [] s
...
let global_section s = section `GlobalSection (vec (at global)) [] s
...
let export_section s = section `ExportSection (vec (at export)) [] s
...
let code _ s =
  ...
  require
    (I64.lt_u (List.fold_left I64.add 0L ns) 0x1_0000_0000L)
...
let code_section s = section `CodeSection (vec (at (sized code))) [] s
...
let elem_section s = section `ElemSection (vec (at elem)) [] s
...
let data_section s = section `DataSection (vec (at data)) [] s
...
let module_ s =
  ...
  require
    (List.length func_types = List.length func_bodies)
    s (len s) "function and code section have inconsistent lengths";
  require
    (data_count = None || data_count = Some (Lib.List32.length datas))
