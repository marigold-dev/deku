...
type stream = { buf : Buffer.t; patches : (int * char) list ref }
...
struct
  ...
  let u32 i =
    Int32.(
      u16 (to_int (logand i 0xffffl));
      u16 (to_int (shift_right i 16)))

  let u64 i =
    Int64.(
      u32 (to_int32 (logand i 0xffffffffL));
  ...
  let rec vu64 i =
    ...
    if 0L <= i && i < 128L then u8 b
      ...
      u8 (b lor 0x80);
      vu64 (Int64.shift_right_logical i 7))
  ...
  let len i =
    if Int32.to_int (Int32.of_int i) <> i then
      Code.error Source.no_region "cannot encode length with more than 32 bit";
    vu32 (Int32.of_int i)
  ...
  let patch_gap32 p n =
    assert (n <= 0x0fff_ffff);
    (* Strings cannot excess 2G anyway *)
  ...
  let vec_type = function V128Type -> vs7 (-0x05)
  ...
  let memory_type = function MemoryType lim -> limits vu32 lim
  let mutability = function Immutable -> u8 0 | Mutable -> u8 1
  ...
  let rec instr e =
    ...
    | ElemDrop x ->
        ...
        vu32 0x0dl;
        var x
    | Load ({ ty = I32Type; pack = None; _ } as mo) ->
        op 0x28;
        memop mo
    | Load ({ ty = I64Type; pack = None; _ } as mo) ->
        op 0x29;
        memop mo
    | Load ({ ty = F32Type; pack = None; _ } as mo) ->
        op 0x2a;
        memop mo
    | Load ({ ty = F64Type; pack = None; _ } as mo) ->
        op 0x2b;
        memop mo
    ...
    | Load ({ ty = I32Type; pack = Some (Pack16, ZX); _ } as mo) ->
        op 0x2f;
        memop mo
    | Load { ty = I32Type; pack = Some (Pack32, _); _ } -> assert false
    ...
    | Load ({ ty = I64Type; pack = Some (Pack32, ZX); _ } as mo) ->
        op 0x35;
        memop mo
    | Load { ty = F32Type | F64Type; pack = Some _; _ } -> assert false
    | Load { ty = I32Type | I64Type; pack = Some (Pack64, _); _ } ->
        assert false
    | Store ({ ty = I32Type; pack = None; _ } as mo) ->
        op 0x36;
        memop mo
    | Store ({ ty = I64Type; pack = None; _ } as mo) ->
        op 0x37;
        memop mo
    | Store ({ ty = F32Type; pack = None; _ } as mo) ->
        op 0x38;
        memop mo
    | Store ({ ty = F64Type; pack = None; _ } as mo) ->
        op 0x39;
        memop mo
    | Store ({ ty = I32Type; pack = Some Pack8; _ } as mo) ->
        op 0x3a;
        memop mo
    | Store ({ ty = I32Type; pack = Some Pack16; _ } as mo) ->
        op 0x3b;
        memop mo
    | Store { ty = I32Type; pack = Some Pack32; _ } -> assert false
    | Store ({ ty = I64Type; pack = Some Pack8; _ } as mo) ->
        op 0x3c;
        memop mo
    | Store ({ ty = I64Type; pack = Some Pack16; _ } as mo) ->
        op 0x3d;
        memop mo
    | Store ({ ty = I64Type; pack = Some Pack32; _ } as mo) ->
        op 0x3e;
        memop mo
    ...
    | VecLoad ({ ty = V128Type; pack = None; _ } as mo) ->
        ...
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, SX)); _ } as
        mo) ->
        vecop 0x01l;
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack8x8, ZX)); _ } as
        mo) ->
        vecop 0x02l;
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, SX)); _ } as
        mo) ->
        vecop 0x03l;
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack16x4, ZX)); _ } as
        mo) ->
        vecop 0x04l;
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, SX)); _ } as
        mo) ->
        vecop 0x05l;
        memop mo
    | VecLoad
        ({ ty = V128Type; pack = Some (Pack64, ExtLane (Pack32x2, ZX)); _ } as
        mo) ->
        vecop 0x06l;
        memop mo
    ...
    | Convert (I32 I32Op.TruncSatSF32) ->
        op 0xfc;
        vu32 0x00l
    | Convert (I32 I32Op.TruncSatUF32) ->
        op 0xfc;
        vu32 0x01l
    | Convert (I32 I32Op.TruncSatSF64) ->
        op 0xfc;
        vu32 0x02l
    | Convert (I32 I32Op.TruncSatUF64) ->
        op 0xfc;
        vu32 0x03l
    ...
    | Convert (I64 I64Op.TruncSatSF32) ->
        op 0xfc;
        vu32 0x04l
    | Convert (I64 I64Op.TruncSatUF32) ->
        op 0xfc;
        vu32 0x05l
    | Convert (I64 I64Op.TruncSatSF64) ->
        op 0xfc;
        vu32 0x06l
    | Convert (I64 I64Op.TruncSatUF64) ->
        op 0xfc;
        vu32 0x07l
    ...
    | VecExtract (V128 (I8x16 (V128Op.Extract (i, SX)))) ->
        vecop 0x15l;
        u8 i
    | VecExtract (V128 (I8x16 (V128Op.Extract (i, ZX)))) ->
        vecop 0x16l;
        u8 i
    | VecExtract (V128 (I16x8 (V128Op.Extract (i, SX)))) ->
        vecop 0x18l;
        u8 i
    | VecExtract (V128 (I16x8 (V128Op.Extract (i, ZX)))) ->
        vecop 0x19l;
        u8 i
    | VecExtract (V128 (I32x4 (V128Op.Extract (i, ())))) ->
        vecop 0x1bl;
        u8 i
    | VecExtract (V128 (I64x2 (V128Op.Extract (i, ())))) ->
        vecop 0x1dl;
        u8 i
    | VecExtract (V128 (F32x4 (V128Op.Extract (i, ())))) ->
        vecop 0x1fl;
        u8 i
    | VecExtract (V128 (F64x2 (V128Op.Extract (i, ())))) ->
        vecop 0x21l;
        u8 i
    | VecReplace (V128 (I8x16 (V128Op.Replace i))) ->
        vecop 0x17l;
        u8 i
    | VecReplace (V128 (I16x8 (V128Op.Replace i))) ->
        vecop 0x1al;
        u8 i
    | VecReplace (V128 (I32x4 (V128Op.Replace i))) ->
        vecop 0x1cl;
        u8 i
    | VecReplace (V128 (I64x2 (V128Op.Replace i))) ->
        vecop 0x1el;
        u8 i
    | VecReplace (V128 (F32x4 (V128Op.Replace i))) ->
        vecop 0x20l;
        u8 i
    | VecReplace (V128 (F64x2 (V128Op.Replace i))) ->
        vecop 0x22l;
        u8 i
  ...
  let type_section ts = section 1 (vec type_) ts (ts <> [])
  ...
  let import_section ims = section 2 (vec import) ims (ims <> [])

  (* Function section *)
  let func f = var f.it.ftype
  let func_section fs = section 3 (vec func) fs (fs <> [])
  ...
  let table_section tabs = section 4 (vec table) tabs (tabs <> [])
  ...
  let memory_section mems = section 5 (vec memory) mems (mems <> [])
  ...
  let global_section gs = section 6 (vec global) gs (gs <> [])
  ...
  let export_section exs = section 7 (vec export) exs (exs <> [])
  ...
  let start_section xo = section 8 (opt start) xo (xo <> None)
  ...
  let code_section fs = section 10 (vec code) fs (fs <> [])

  (* Element section *)
  let is_elem_kind = function FuncRefType -> true | _ -> false
  ...
  let elem_section elems = section 9 (vec elem) elems (elems <> [])
