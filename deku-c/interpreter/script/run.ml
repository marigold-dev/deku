...
let output_file =
  dispatch_file_ext create_binary_file create_sexpr_file
...
let input_file file run =
  dispatch_file_ext input_binary_file
...
let rec run_command cmd =
  ...
  | Assertion ass ->
      quote := cmd :: !quote;
      if not !Flags.dry then run_assertion ass
...
and run_meta cmd =
  ...
  | Input (x_opt, file) ->
      (try
         if not (input_file file run_quote_script) then
...
and run_script script = List.iter run_command script
