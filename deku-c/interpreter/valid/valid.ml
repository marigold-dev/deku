...
let refer category (s : Free.Set.t) x =
  if not (Free.Set.mem x.it s) then
    error x.at ("undeclared " ^ category ^ " reference " ^ Int32.to_string x.it)
...
let check_stack ts1 ts2 at =
  require
    (List.length ts1 = List.length ts2 && List.for_all2 eq_ty ts1 ts2)
    at
...
let type_vec_lane = function Values.V128 laneop -> V128.type_of_lane laneop
...
let type_cvtop at = function
  | Values.I32 cvtop ->
      ...
      ( (match cvtop with
        ...
        | WrapI64 -> I64Type
        | TruncSF32 | TruncUF32 | TruncSatSF32 | TruncSatUF32 | ReinterpretFloat
          ->
            F32Type
  ...
  | Values.I64 cvtop ->
      ...
      ( (match cvtop with
        ...
        | TruncSF64 | TruncUF64 | TruncSatSF64 | TruncSatUF64 | ReinterpretFloat
          ->
            F64Type),
        I64Type )
  | Values.F32 cvtop ->
...
let num_lanes = function Values.V128 laneop -> V128.num_lanes laneop
...
let check_vec_binop binop at =
  ...
  | Values.(V128 (V128.I8x16 (V128Op.Shuffle is))) ->
      if List.exists (( <= ) 32) is then error at "invalid lane index"
  | _ -> ()
...
let check_memop (c : context) (memop : ('t, 's) memop) ty_size get_sz at =
  ...
  in
  require
    (1 lsl memop.align <= size)
    at "alignment must not be larger than natural"
...
let rec check_instr (c : context) (e : instr) (s : infer_result_type) : op_type
  ...
  | Select None ->
      let t = peek 1 s in
      require
        (match t with None -> true | Some t -> is_num_type t || is_vec_type t)
        e.at
  ...
  | Select (Some ts) ->
      require
        (List.length ts = 1)
        e.at "invalid result arity other than 1 is not (yet) allowed";
  ...
  | VecLoadLane (memop, i) ->
      ...
      require
        (i < vec_size memop.ty / packed_size memop.pack)
        e.at "invalid lane index";
  ...
  | RefIsNull ->
      let t = peek 0 s in
      require
        (match t with None -> true | Some t -> is_ref_type t)
        e.at
  ...
  | VecExtract extractop ->
      ...
      require
        (lane_extractop extractop < num_lanes extractop)
        e.at "invalid lane index";
      [ t ] --> [ NumType t2 ]
  | VecReplace replaceop ->
      ...
      require
        (lane_replaceop replaceop < num_lanes replaceop)
        e.at "invalid lane index";
      [ t; NumType t2 ] --> [ t ]
...
let check_const (c : context) (const : const) (t : value_type) =
  require
    ...
    const.at "constant expression required";
...
let check_start (c : context) (start : start) =
  ...
  require
    ...
    start.at "start function must not have parameters or results"

let check_import (im : import) (c : context) : context =
  let { module_name = _; item_name = _; idesc } = im.it in
  match idesc.it with
  | FuncImport x -> { c with funcs = type_ c x :: c.funcs }
  | TableImport tt ->
...
let check_module (m : module_) =
  ...
  require
    (List.length c.memories <= 1)
    m.at "multiple memories are not allowed (yet)"
