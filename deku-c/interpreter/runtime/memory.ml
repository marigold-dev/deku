...
let create n =
  if I32.gt_u n 0x10000l then raise SizeOverflow
  else
    try
...
let grow mem delta =
  ...
  if I32.gt_u old_size new_size then raise SizeOverflow
    ...
    if not (valid_limits lim') then raise SizeLimit
      ...
      Array1.blit
        (Array1_64.sub mem.content 0L dim)
        (Array1_64.sub after 0L dim);
...
let load_vec mem a o t =
  ...
  | V128Type ->
      V128
        (V128.of_bits
           (load_bytes mem (effective_address a o) (Types.vec_size t)))
