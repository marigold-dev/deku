...
docker build -f ../docker/static.Dockerfile -t deku ../
...
# We'll use docker to run the sidecli
sidecli() {
  docker run -v $PWD/data:/app/data deku /app/sidecli "$@"
}
...
node0=$(sidecli setup-identity --uri http://localhost:4440 ./data/0)
node1=$(sidecli setup-identity --uri http://localhost:4441 ./data/1)
node2=$(sidecli setup-identity --uri http://localhost:4442 ./data/2)
...
sidecli setup-tezos ./data/1 \
      --tezos_consensus_contract="$TEZOS_CONSENSUS_ADDRESS" \
      --tezos_rpc_node=$RPC_NODE \
      --tezos_secret="$SECRET_KEY" \
      --unsafe_tezos_required_confirmations 1

sidecli setup-tezos ./data/2 \
      --tezos_consensus_contract="$TEZOS_CONSENSUS_ADDRESS" \
      --tezos_rpc_node=$RPC_NODE \
      --tezos_secret="$SECRET_KEY" \
      --unsafe_tezos_required_confirmations 1
