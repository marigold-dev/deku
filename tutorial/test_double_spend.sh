...
# We don't yet have an indexer ready, but if you observe the block level the transactions were applied
# then you can query the contents of that block. For example:
# BLOCK_LEVEL=<fill in block level>
# curl -d "{\"level\":$BLOCK_LEVEL}" -X POST http://localhost:4440/block-by-level
